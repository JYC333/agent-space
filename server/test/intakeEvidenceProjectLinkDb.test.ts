...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[intake-evidence-project-link-db] skipped — Docker/Postgres unavailable: ${
...
beforeEach(async () => {
  ...
  await pool.query(
    `TRUNCATE evidence_links, extracted_evidence, source_snapshots, intake_items, workspace_source_bindings,
...
async function seedItemWithEvidence(connectionId: string | null = CONNECTION): Promise<{ itemId: string; evidenceId: string }> {
  ...
  await pool!.query(
    `INSERT INTO intake_items (
     ...
     ) VALUES ($1,$2,$3,'external_url','New paper',$4,$4,'new','unread','excerpt_saved','summary_only',$4,$4)`,
  ...
  await pool!.query(
    `INSERT INTO extracted_evidence (
     ...
     ) VALUES ($1,$2,$3,'intake_item',$3,'excerpt','New paper','Abstract text','connection_scan','normal',
...
async function seedSourceSnapshot(itemId: string, connectionId: string): Promise<void> {
  ...
  await pool!.query(
    `INSERT INTO source_snapshots (
       id, space_id, intake_item_id, connection_id, snapshot_type, content_hash,
...
describe("Evidence→project auto-link (real Postgres)", () => {
  it("links new evidence to the bound project and is idempotent on re-run", async () => {
    ...
    const created = await linkEvidenceToBoundProjects(pool!, { spaceId: SPACE, intakeItemId: itemId });
  ...
  it("auto-linked evidence is returned by context evidence selection for the project", async () => {
    ...
    const { itemId, evidenceId } = await seedItemWithEvidence();
    await linkEvidenceToBoundProjects(pool!, { spaceId: SPACE, intakeItemId: itemId });
