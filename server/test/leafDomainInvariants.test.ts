...
describe("Leaf domain ownership invariants", () => {
  it("registers activity, intake, knowledge, and tasks as server-owned modules", () => {
    expect(SERVER_MODULES.map((module) => module.name)).toEqual(
  ...
  it("applies knowledge proposals in the server without auto-promoting knowledge into memory", () => {
  ...
  it("registers follow_up_task applier in the server and creates a task row without direct memory writes", () => {
