...
import {
  ...
  dedupeDomains,
  PROVENANCE_CAP,
} from "../src/modules/brainThink/aggregate";
...
function section(over: Partial<BrainThinkDomainSection> & Pick<BrainThinkDomainSection, "domain">): BrainThinkDomainSection {
  return {
    domain: over.domain,
...
describe("brainThink aggregate", () => {
  ...
  it("suppresses all follow-ups when the viewer lacks Brain Ops scan authority", () => {
    const gapWithStale = aggregateGaps([
      section({ domain: "knowledge", brief: brief({ gap: { stale: [{ object_type: "knowledge_item", object_id: "k1", title: "K1", reason: "old" }] } }) }),
    ]);
...
describe("BrainThinkResponseSchema", () => {
  it("accepts a well-formed read-only response and rejects a canonical-write claim", async () => {
    ...
    expect(BrainThinkResponseSchema.safeParse(base).success).toBe(true);
    expect(BrainThinkResponseSchema.safeParse({ ...base, canonical_write_performed: true }).success).toBe(false);
  });
});
