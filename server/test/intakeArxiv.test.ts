...
import {
  ...
  parseArxivFeed,
  parseArxivReference,
} from "../src/modules/intake/connectors/arxiv";
import {
  __setArxivThrottleForTests,
  acquireArxivRequestSlot,
} from "../src/modules/intake/connectors/arxivThrottle";
import {
  normalizeArxivQueryConfig,
  listSourcePresets,
  SourcePresetService,
} from "../src/modules/intake/sourcePresets/service";
...
describe("intake source preset routes", () => {
  ...
  it("returns 401 to unauthenticated preset requests", async () => {
    ...
    for (const request of [
      { method: "GET" as const, url: "/api/v1/intake/source-presets" },
      { method: "POST" as const, url: "/api/v1/intake/source-presets/arxiv/preview" },
      { method: "POST" as const, url: "/api/v1/intake/source-presets/arxiv" },
    ]) {
  ...
  it("lists the arXiv preset for authenticated users", async () => {
    ...
    const res = await app.inject({ method: "GET", url: "/api/v1/intake/source-presets" });
...
describe("IntakeExtractionWorker arXiv HTML-first extraction", () => {
  it("extracts arXiv abs URLs from the HTML rendition first", async () => {
    ...
    expect(artifactInserts[0]?.sql).toContain("'intake_reader_document'");
  ...
  it("falls back to the PDF rendition when the HTML rendition is unavailable", async () => {
    ...
    expect(artifactInserts[0]?.sql).toContain("'intake_raw_snapshot'");
    expect(artifactInserts[0]?.params[4]).toBe("application/pdf");
    expect(artifactInserts[1]?.sql).toContain("'intake_reader_document'");
...
class ExtractionDb implements Queryable {
  ...
  async query<Row = Record<string, unknown>>(sql: string, params: readonly unknown[] = []) {
    ...
    if (
      ...
      sql.includes("UPDATE intake_items") ||
