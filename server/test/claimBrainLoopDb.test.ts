...
import { buildClaimTrajectory, scanClaimContradictions } from "../src/modules/knowledge/claimBrainLoop";

// Real-PostgreSQL coverage for the Slice E claim brain loop. FakeDb unit tests
...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[claim-brain-loop-db] skipped — Docker/Postgres unavailable: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}, 180_000);
...
beforeEach(async () => {
  ...
  await pool.query(
    `INSERT INTO spaces (id, name, type, created_by_user_id, created_at, updated_at)
     VALUES ($1, 'Brain Loop Space', 'household', $2, now(), now())`,
    [SPACE, VIEWER],
  );
...
describe("Slice E claim brain loop (real Postgres)", () => {
