...
import { runBrainOpsDreamCycleV2 } from "../src/modules/brainOps/dreamCycle";
...
vi.mock("../src/modules/brainOps/service", () => ({
  BrainOpsService: vi.fn().mockImplementation(() => ({
...
class DreamCycleFakeDb implements Queryable {
...
describe("Brain Ops Dream Cycle Lite v2 service", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("does not create review packets when create_packets is false", async () => {
    const db = new DreamCycleFakeDb();
    const result = await runBrainOpsDreamCycleV2(db, {
      spaceId: "space-1",
      userId: "user-1",
      request: {
        window_days: 14,
    ...
    expect(db.artifactInserts[0]).toMatchObject({
      artifact_type: "brain_ops_dream_cycle_v2_report",
    });
  });
  ...
  it("does not include recent private briefs in space_ops claim candidate packets", async () => {
    const db = new DreamCycleFakeDb();
    await runBrainOpsDreamCycleV2(db, {
      spaceId: "space-1",
      userId: "user-1",
      request: {
        window_days: 14,
  ...
  it("keeps the Dream Cycle report when optional claim packet creation fails", async () => {
