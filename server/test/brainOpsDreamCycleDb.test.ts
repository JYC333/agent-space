...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[brain-ops-dream-cycle-db] skipped — Docker/Postgres unavailable: ${
...
beforeEach(async () => {
  ...
  await pool.query(
    `INSERT INTO spaces (id, name, type, created_at, updated_at)
     VALUES ($1, 'Dream Cycle DB', 'personal', now(), now())`,
    [SPACE],
  );
  await pool.query(
...
describe("Brain Ops Dream Cycle Lite v2 (real Postgres)", () => {
  it("persists maintenance, diagnostics, claim packet, and Dream Cycle report without canonical writes", async () => {
    if (!available || !pool) return;
    await seedKnowledge();
    const briefArtifactId = await seedRecentBrief();

    const result = await runBrainOpsDreamCycleV2(pool, {
      spaceId: SPACE,
      userId: USER,
      request: {
        window_days: 14,
    ...
    expect(artifactCounts.brain_ops_dream_cycle_v2_report).toBe(1);
