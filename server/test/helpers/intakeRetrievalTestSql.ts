...
export function handleIntakeRetrievalTestSql<Row = Record<string, unknown>>(
  sql: string,
  ...
  if (
    ...
    norm.includes("FROM intake_items")
  ) {
    return empty<Row>();
  }
  if (
    norm.startsWith("SELECT ee.id, ee.intake_item_id, ss.connection_id AS source_snapshot_connection_id,") &&
  ...
  if (
    ...
    norm.startsWith("SELECT intake_item_id FROM extracted_evidence")
  ) {
    return empty<Row>();
  }
  if (
