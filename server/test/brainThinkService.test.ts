...
import { BrainThinkService } from "../src/modules/brainThink";
...
function cannedBrief(domain: BrainThinkDomain, opts: { stale?: boolean; itemId?: string; sourceId?: string } = {}): RetrievalBriefResponse {
...
function typeFor(domain: BrainThinkDomain) {
  return domain === "memory" ? "memory_entry" : domain === "project" ? "project_public_summary" : "knowledge_item";
}

describe("BrainThinkService.think (orchestration)", () => {
  it("fans across domains, aggregates gaps/provenance, logs Memory reads, and gates follow-ups", async () => {
    ...
    const service = new BrainThinkService(db, CONFIG, {
    ...
    expect(inserts.filter((q) => q.params.includes("brain_think_session"))).toHaveLength(1);
  ...
  it("suppresses follow-ups when the viewer lacks Brain Ops scan authority", async () => {
    const db = new FakeDb();
    const service = new BrainThinkService(db, CONFIG, {
