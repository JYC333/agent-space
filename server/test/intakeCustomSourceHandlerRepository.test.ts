...
// test/fixtures/intakeCustomSourceHandlersSchema.sql.
//
// Skips gracefully when Docker is unavailable so `npm test` runs everywhere.

const SCHEMA = readFileSync(
  join(process.cwd(), "test/fixtures/intakeCustomSourceHandlersSchema.sql"),
  "utf8",
);
...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[intake-custom-source-handler-repository] skipped — Docker/Postgres unavailable: ${
