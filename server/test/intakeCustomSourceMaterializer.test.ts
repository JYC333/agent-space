...
// constraints in particular — see test/fixtures/intakeCustomSourceMaterializerSchema.sql)
...
const SCHEMA = readFileSync(
  join(process.cwd(), "test/fixtures/intakeCustomSourceMaterializerSchema.sql"),
  "utf8",
);
...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[intake-custom-source-materializer] skipped — Docker/Postgres unavailable: ${
...
describe("CustomSourceMaterializationService (real Postgres)", () => {
  it("a validation failure writes no Intake rows and marks the run validation_failed", async () => {
    ...
    const items = await pool!.query(`SELECT count(*)::int AS n FROM intake_items`);
  ...
  it("a valid output writes intake_items, source_snapshots, extracted_evidence, and artifacts, and marks the run succeeded", async () => {
    if (!available || !service) return;
    const { connId, runId } = await seedRun();
    ...
    const item = await pool!.query<{ source_external_id: string; content_state: string; retention_policy: string }>(
      `SELECT source_external_id, content_state, retention_policy FROM intake_items WHERE space_id = $1`,
      [SPACE_A],
    );
  ...
  it("stores excerpt-only output as excerpt_saved so full-text extraction remains available", async () => {
    ...
    const item = await pool!.query<{ content_state: string; excerpt: string | null }>(
      `SELECT content_state, excerpt FROM intake_items WHERE space_id = $1`,
      [SPACE_A],
    );
  ...
  it("repairs previously misclassified excerpt-only items on re-materialization", async () => {
    ...
    await pool!.query(
      `INSERT INTO intake_items (
         ...
         content_hash, excerpt, status, read_status, content_state,
         ...
         'old-hash', 'Old excerpt', 'new', 'unread', 'content_saved',
    ...
    const item = await pool!.query<{ content_state: string }>(
      `SELECT content_state FROM intake_items WHERE connection_id = $1 AND source_external_id = 'article-1'`,
      [connId],
    );
  ...
  it("labels Level 2 Source Recipe materialization separately from handler runs", async () => {
    ...
    const item = await pool!.query<{ metadata_json: Record<string, unknown> }>(
      `SELECT metadata_json FROM intake_items WHERE connection_id = $1`,
      [connId],
    );
    ...
    expect(artifacts.rows.map((row) => row.artifact_type).sort()).toEqual([
      "intake_source_recipe_output",
      "intake_source_recipe_snapshot",
    ]);
  ...
  it("re-materializing the same external_id updates the existing item instead of duplicating it", async () => {
    ...
    const items = await pool!.query(`SELECT count(*)::int AS n FROM intake_items WHERE space_id = $1`, [SPACE_A]);
  ...
  it("writes the policy envelope's retention_policy instead of a hardcoded value, on both insert and update", async () => {
    ...
    const afterInsert = await pool!.query<{
      ...
      `SELECT retention_policy, content_state, excerpt, metadata_json FROM intake_items WHERE space_id = $1`,
      [SPACE_A],
    );
    ...
    const afterUpdate = await pool!.query<{ retention_policy: string }>(
      `SELECT retention_policy FROM intake_items WHERE space_id = $1`,
      [SPACE_A],
    );
