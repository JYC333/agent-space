...
const dreamCycleMock = vi.hoisted(() => vi.fn());

vi.mock("../src/db/pool", () => ({
  getDbPool: vi.fn(),
}));

vi.mock("../src/modules/brainOps/dreamCycle", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../src/modules/brainOps/dreamCycle")>();
  return {
    ...actual,
    runBrainOpsDreamCycleV2: dreamCycleMock,
  };
});
    ...
    if (/FROM space_retrieval_settings/.test(norm)) {
      return {
        rows: [{
          ...
          brain_ops_review_mode: this.brainOpsReviewMode,
          brain_ops_scan_mode: this.brainOpsScanMode,
...
describe("Brain Ops routes", () => {
  ...
  it("rejects invalid windows before querying the read model", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/summary?window_days=0",
    });
  ...
  it("requires owner/admin or enabled member Brain Ops access", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/summary",
    });

    expect(res.statusCode).toBe(403);
    expect(res.json()).toEqual({ detail: "Requires space owner/admin role or enabled Brain Ops member review/scan access" });
    expect(db.calls).toHaveLength(1);
  });

  it("allows members when Brain Ops member review is enabled", async () => {
    __setAuthRepositoryForTests(auth("member"));
    const db = new EmptyBrainOpsDb();
    db.brainOpsReviewMode = "members";
    vi.mocked(getDbPool).mockReturnValue(db as never);
    app = buildServer(config(), { logger: false });
  ...
  it("allows reviewers and rejects guests when Brain Ops member review is enabled", async () => {
    __setAuthRepositoryForTests(auth("reviewer"));
    const reviewerDb = new EmptyBrainOpsDb();
    reviewerDb.brainOpsReviewMode = "members";
    ...
    const guestDb = new EmptyBrainOpsDb();
    guestDb.brainOpsReviewMode = "members";
  ...
  it("serves a drill-down section for an authorized operator", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/drilldown?section=index_freshness&limit=10",
    });
  ...
  it("serves the embedding backlog drill-down section for an authorized operator", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/drilldown?section=embedding_backlog&limit=10",
    });
  ...
  it("runs Dream Cycle Lite v2 for an authorized scanner", async () => {
    __setAuthRepositoryForTests(auth("admin"));
    const db = new EmptyBrainOpsDb();
    dreamCycleMock.mockResolvedValue(dreamCycleResponse());
    vi.mocked(getDbPool).mockReturnValue(db as never);
    app = buildServer(config(), { logger: false });

    const res = await app.inject({
      method: "POST",
      url: "/api/v1/brain-ops/dream-cycle-v2",
    ...
    expect(dreamCycleMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
      spaceId: "space-1",
      userId: "user-1",
  ...
  it("blocks Dream Cycle space_ops scope when review mode is private-only", async () => {
    __setAuthRepositoryForTests(auth("admin"));
    const db = new EmptyBrainOpsDb();
    vi.mocked(getDbPool).mockReturnValue(db as never);
    app = buildServer(config(), { logger: false });
    ...
    expect(res.statusCode).toBe(403);
    expect(dreamCycleMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown drill-down section before querying", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/drilldown?section=secret_dump",
    });
  ...
  it("denies drill-down to members without Brain Ops access", async () => {
    __setAuthRepositoryForTests(auth("member"));
    const db = new EmptyBrainOpsDb();
    vi.mocked(getDbPool).mockReturnValue(db as never);
    app = buildServer(config(), { logger: false });
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/brain-ops/drilldown?section=source_warnings",
    });
  ...
  it("keeps admins mode restricted to owners and admins", async () => {
    __setAuthRepositoryForTests(auth("admin"));
    const adminDb = new EmptyBrainOpsDb();
    adminDb.brainOpsReviewMode = "admins";
    ...
    __setAuthRepositoryForTests(auth("member"));
    const memberDb = new EmptyBrainOpsDb();
    memberDb.brainOpsReviewMode = "admins";
