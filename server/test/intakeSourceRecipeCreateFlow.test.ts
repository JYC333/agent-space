...
import { listSourceRuns } from "../src/modules/intake/sourceRunReadModel";
...
beforeAll(async () => {
  try {
    ...
    console.warn(
      `[intake-source-recipe-create-flow] skipped - Docker/Postgres unavailable: ${
...
describe("SourceRecipeCreateService (real Postgres)", () => {
  it("plans, creates, dry-runs, activates, and scans a recipe source into Intake", async () => {
    if (!available) return;
    ...
    const items = await pool!.query<{
      ...
      `SELECT title, source_external_id, content_state, metadata_json FROM intake_items WHERE connection_id = $1 ORDER BY title ASC`,
      [created.connection.id],
    );
    ...
    const firstItem = await pool!.query<{ id: string }>(
      `SELECT id FROM intake_items WHERE connection_id = $1 ORDER BY title ASC LIMIT 1`,
      [created.connection.id],
    );
    await repo.createEvidence(IDENTITY, {
      intake_item_id: firstItem.rows[0]!.id,
