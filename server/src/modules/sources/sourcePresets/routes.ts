...
export function registerSourcePresetRoutes(app: FastifyInstance, context: ModuleContext): void {
  ...
  app.get("/api/v1/sources/source-presets", async (request, reply) => {
  ...
  app.post("/api/v1/sources/source-presets/arxiv/preview", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
  ...
  app.post("/api/v1/sources/source-presets/arxiv", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const gate = await enforceSources(context, identity, "source.connection_manage", "source_connection");
