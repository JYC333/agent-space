...
import { ITEM_COLUMNS } from "./sourceRepositoryRows";
...
const ANNOTATION_COLUMNS = `id, space_id, source_item_id, artifact_id, source_snapshot_id,
...
function annotationDocumentTarget(
  ...
  if (ann.source_item_id) return { documentType: "source_item", documentId: ann.source_item_id };
...
export class PgAnnotationRepository {
  ...
  async listAnnotations(
    ...
    let col: string;
    if (documentType === "source_item") col = "source_item_id";
  ...
  async createAnnotation(
    ...
    const nonNullCount = [sourceItemId, artifactId, sourceSnapshotId].filter(Boolean).length;
    if (nonNullCount !== 1) {
      throw new HttpError(422, "Exactly one of source_item_id, artifact_id, source_snapshot_id must be provided");
    }

    // Verify the caller can read the target document before creating an annotation on it.
    const documentType = sourceItemId ? "source_item" : artifactId ? "artifact" : "source_snapshot";
    const documentId = (sourceItemId ?? artifactId ?? sourceSnapshotId)!;
    await assertDocumentReadable(this.db, identity, documentType, documentId);
    ...
    const result = await this.db.query<ReaderAnnotationRow>(
      `INSERT INTO reader_annotations (
         id, space_id, source_item_id, artifact_id, source_snapshot_id,
         ...
         $1, $2, $3, $4, $5,
         $6, $7, $8::jsonb, $9, $10,
         $11, 'active', $12, $13, $13, $14, $14
       ) RETURNING ${ANNOTATION_COLUMNS}`,
      [
        id, identity.spaceId, sourceItemId, artifactId, sourceSnapshotId,
...
async function loadAnnotationConnectionPolicy(
  ...
  if (ann.artifact_id) {
    // Resolve connection by tracing artifact back to a source snapshot or source item.
    const r = await db.query<{ policy_json: unknown }>(
      `SELECT sc.policy_json
         FROM (
           ...
           UNION ALL
           SELECT ii.connection_id FROM source_items ii
            WHERE ii.space_id = $1
...
export class PgReaderActionRepository {
  ...
  async listProjectAnnotations(
    ...
    const cols = `ra.id, ra.space_id, ra.source_item_id, ra.artifact_id, ra.source_snapshot_id,
