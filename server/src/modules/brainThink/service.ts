import type {
  BrainThinkClaimTrajectory,
  BrainThinkDomain,
  BrainThinkDomainSection,
...
import { canInitiateBrainOpsScan } from "../brainOps/reviewPolicy";
import { buildClaimTrajectory } from "../knowledge/claimBrainLoop";
import { persistBrainThinkSessionArtifact } from "./sessionArtifact";
...
const DOMAIN_CONFIG: Record<BrainThinkDomain, DomainConfig> = {
...
/**
 * Think / Ask Brain orchestrator (Slice A). Runs each requested domain's own
 ...
 */
export interface BrainThinkDeps {
  ...
  loadClaimTrajectory?: (claimId: string, spaceId: string, userId: string) => Promise<BrainThinkClaimTrajectory | null>;
}

export class BrainThinkService {
  constructor(
    private readonly db: Queryable,
    private readonly config: ServerConfig,
    private readonly deps: BrainThinkDeps = {},
  ) {}

  static fromConfig(config: ServerConfig): BrainThinkService {
    return new BrainThinkService(dbPool(config), config);
  }

  async think(input: BrainThinkInput): Promise<BrainThinkResponse> {
    const now = new Date();
    ...
    const briefArtifactRefs = sections
      .filter((section): section is BrainThinkDomainSection & { artifact_id: string } =>
    ...
    // Brain Ops scan authority, so only offer them when the viewer actually has
    ...
    const canRunActions = await (this.deps.canRunActions
      ...
      : canInitiateBrainOpsScan(this.db, input.spaceId, input.userId));
    ...
    if (input.persist) {
      try {
        sessionArtifactId = await persistBrainThinkSessionArtifact(this.db, {
        ...
        sessionArtifactError = "brain_think_session_persist_failed";
      }
    }
  ...
  private async collectClaimTrajectories(
    input: BrainThinkInput,
    provenance: readonly BrainThinkProvenanceItem[],
  ): Promise<BrainThinkClaimTrajectory[]> {
    const claimIds: string[] = [];
    for (const item of provenance) {
      if (item.object_type === "claim" && !claimIds.includes(item.object_id)) claimIds.push(item.object_id);
    }
    const out: BrainThinkClaimTrajectory[] = [];
  ...
  private async defaultLoadClaimTrajectory(
    ...
    spaceId: string,
    userId: string,
  ): Promise<BrainThinkClaimTrajectory | null> {
  ...
  private async buildCombinedAnswer(
    input: BrainThinkInput,
    sections: readonly BrainThinkDomainSection[],
    ctx: DomainCtx,
  ): Promise<string | null> {
  ...
  private async buildDomainSection(
    ...
    input: BrainThinkInput,
    ctx: DomainCtx,
  ): Promise<BrainThinkDomainSection> {
...
function combinedSynthesisCandidates(
  ...
  for (const section of sections) {
    ...
    out.push({
      ...
      objectId: `brain-think-${section.domain}`,
...
function combinedCandidateText(section: BrainThinkDomainSection, answer: string): string {
...
function primaryObjectType(section: BrainThinkDomainSection): RetrievalObjectType {
...
function domainLabel(domain: BrainThinkDomain): string {
  if (domain === "memory") return "Memory";
  if (domain === "project") return "Project summaries";
  return "Knowledge";
}

function sourceConnectionIdsFromItems(items: readonly BrainThinkDomainSection["items"][number][]): string[] {
  const ids: string[] = [];
