...
export const DEFAULT_DOMAINS: BrainThinkDomain[] = ["knowledge"];

/** Bound how many cited sources cross into the provenance list. */
export const PROVENANCE_CAP = 60;

export function dedupeDomains(domains: BrainThinkDomain[]): BrainThinkDomain[] {
  const seen = new Set<BrainThinkDomain>();
  const out: BrainThinkDomain[] = [];
...
export function aggregateGaps(sections: BrainThinkDomainSection[]): BrainThinkGapSummary {
  let stale = 0;
  let thin = 0;
  let uncited = 0;
  let contradictions = 0;
  let missing = 0;
  const lowCoverage: BrainThinkDomain[] = [];
...
export function collectProvenance(sections: BrainThinkDomainSection[]): BrainThinkProvenanceItem[] {
  const out: BrainThinkProvenanceItem[] = [];
 ...
 * routes require Brain Ops scan authority, so follow-ups are only offered when
...
export function buildFollowUps(
  ...
  gap: BrainThinkGapSummary,
  canRunActions: boolean,
): BrainThinkFollowUp[] {
  if (!canRunActions) return [];
  const followUps: BrainThinkFollowUp[] = [];
