...
import { BrainThinkService } from "./service";

export function registerRoutes(app: FastifyInstance, context: ModuleContext): void {
  // Unified Ask Brain / Think entry point. Read-only and proposal-first: it
  ...
  app.post("/api/v1/brain/think", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const protocol = await loadProtocol();
      const parsed = protocol.BrainThinkRequestSchema.safeParse(jsonBody(request));
      if (!parsed.success) {
        throw new HttpError(422, validationMessage(parsed.error.issues));
      }
      const service = BrainThinkService.fromConfig(context.config);
      ...
      return reply.send(protocol.BrainThinkResponseSchema.parse(result));
    } catch (error) {
