...
export class ProjectResearchMonitorComparisonService {
        ...
        await db.query(
          `INSERT INTO research_paper_cards (
             ...
             provenance_json,edited_by_user,stance,comparison_detail,created_at,updated_at
              ...
              LIMIT 1
           ON CONFLICT (project_id,source_item_id) DO UPDATE SET
  ...
  private async eligiblePapers(spaceId: string, projectId: string, sourceItemIds: string[]) {
    ...
    const rows = await this.db.query<{
      ...
      `SELECT DISTINCT ON (pci.source_item_id) pci.source_item_id,left(si.title,1000) AS title,left(si.excerpt,4000) AS excerpt,
         ...
         LEFT JOIN research_paper_cards pc ON pc.space_id=pci.space_id AND pc.project_id=pci.project_id AND pc.source_item_id=pci.source_item_id
        ...
        ORDER BY pci.source_item_id,pci.updated_at DESC`,
