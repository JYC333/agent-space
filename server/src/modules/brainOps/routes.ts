...
export function registerRoutes(app: FastifyInstance, context: ModuleContext): void {
  app.get("/api/v1/brain-ops/summary", async (request, reply) => {
    ...
    try {
      ...
      const access = await requireSpaceBrainOpsRole(context.config, identity, reply);
      if (!access) return reply;
      const service = BrainOpsService.fromConfig(context.config);
      ...
      return reply.send(protocol.BrainOpsSummarySchema.parse(summary));
    } catch (error) {
      return sendRouteError(reply, error);
    }
  });

  app.get("/api/v1/brain-ops/drilldown", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const protocol = await loadProtocol();
      const q = query(request);
      const section = protocol.BrainOpsDrilldownSectionSchema.safeParse(q.section);
      if (!section.success) {
        throw new HttpError(
          422,
      ...
      return reply.send(protocol.BrainOpsDrilldownSchema.parse(drilldown));
    } catch (error) {
      return sendRouteError(reply, error);
    }
  });

  app.post("/api/v1/brain-ops/dream-cycle-v2", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const protocol = await loadProtocol();
      const parsed = protocol.BrainOpsDreamCycleV2RequestSchema.safeParse(jsonBody(request));
      if (!parsed.success) throw new HttpError(422, validationMessage(parsed.error.issues));
      const access = await requireSpaceBrainOpsRole(context.config, identity, reply);
      if (!access) return reply;
      if (!access.canScan) {
        throw new HttpError(403, "Requires space owner/admin role or enabled Brain Ops member scan access");
      }
      ...
      const result = await withDbTransaction(dbPool(context.config), async (client) =>
        runBrainOpsDreamCycleV2(client, {
          spaceId: identity.spaceId,
          userId: identity.userId,
          request: parsed.data,
        }));
      return reply.code(201).send(protocol.BrainOpsDreamCycleV2ResponseSchema.parse(result));
    } catch (error) {
      return sendRouteError(reply, error);
    }
  });
...
async function requireSpaceBrainOpsRole(
  config: ServerConfig,
  ...
  const canReviewSpaceOps = roleCanReviewSpaceOps(role, settings.brainOpsReviewMode);
  const canScan = roleCanInitiateBrainOpsScan(role, settings.brainOpsScanMode);
  if (!canScan && !canReviewSpaceOps) {
    reply.code(403).send({ detail: "Requires space owner/admin role or enabled Brain Ops member review/scan access" });
    return null;
  }
  return {
