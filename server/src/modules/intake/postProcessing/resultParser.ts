...
export function parsePostProcessingResult(
  ...
  const allowedItemIds = new Set(intakeItemIds);
  ...
  requireUniqueItemIds(itemSummaries.map((item) => item.intake_item_id), "item_summaries");
  requireUniqueItemIds(itemDecisions.map((item) => item.intake_item_id), "item_decisions");
  ...
  if (actions.per_item_summary && intakeItemIds.length > 0 && itemSummaries.length !== intakeItemIds.length) {
  ...
  if (
    ...
    itemDecisions.length !== intakeItemIds.length
  ) {
    throw new HttpError(
      422,
...
function parseItemSummary(
  ...
  const itemId = requiredKnownItemId(record.intake_item_id, `item_summaries[${index}].intake_item_id`, allowedItemIds);
  return {
...
function parseItemDecision(
  ...
  const itemId = requiredKnownItemId(record.intake_item_id, `item_decisions[${index}].intake_item_id`, allowedItemIds);
...
function parseEvidenceCandidate(
  ...
  const itemId = requiredKnownItemId(record.intake_item_id, `evidence_candidates[${index}].intake_item_id`, allowedItemIds);
  return {
    intake_item_id: itemId,
...
function requiredKnownItemId(value: unknown, field: string, allowedItemIds: Set<string>): string {
  ...
  if (!allowedItemIds.has(itemId)) throw new HttpError(422, `${field} must reference an input intake item`);
  return itemId;
}
...
function requireUniqueItemIds(itemIds: string[], field: string): void {
  ...
  for (const itemId of itemIds) {
    if (seen.has(itemId)) throw new HttpError(422, `${field} contains duplicate intake_item_id ${itemId}`);
    seen.add(itemId);
  }
}
