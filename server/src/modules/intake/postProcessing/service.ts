...
import { ITEM_COLUMNS, type EvidenceRow, type IntakeItemRow, type SourceConnectionRow } from "../intakeRepositoryRows";
...
export class SourcePostProcessingService {
  ...
  async decisionAction(
    ...
    if (action === "select" || action === "triage" || action === "ignore") {
      ...
      await repo.setItemStatus(identity.spaceId, decision.intake_item_id, status);
      return {
        decision: await repo.updateDecisionReview({
          spaceId: identity.spaceId,
          decisionId,
          reviewStatus: action === "ignore" ? "ignored" : "accepted",
          appliedItemStatus: status,
    ...
    if (action === "queue_content") {
      const jobIds = await repo.queueFullTextExtractionForItems({
        spaceId: identity.spaceId,
        connection,
        itemIds: [decision.intake_item_id],
        metadata: {
    ...
    if (action === "extract_evidence") {
      const item = await this.loadDecisionItem(identity.spaceId, decision.intake_item_id);
    ...
    if (action === "create_proposal") {
      ...
      const proposalId = await repo.insertProposal({
        ...
        projectId: decision.project_id,
        title: `Review intake candidate: ${item.title}`,
        summary: proposalMarkdown,
        payload: {
          operation: "create",
          proposed_content: proposalMarkdown,
          source_refs: [{ type: "intake_item", id: item.id }],
          metadata: {
    ...
    if (action === "rerun_item") {
      ...
      const batch = await repo.collectInputBatch({
        ...
        inputConfig,
        cursor: null,
        explicitItemIds: [decision.intake_item_id],
      });
  ...
  async runOneOff(
    ...
    const itemIds = stringList(body.intake_item_ids);
    const evidenceIds = stringList(body.evidence_ids);
    if (!itemIds.length && !evidenceIds.length) {
      throw new HttpError(422, "At least one intake_item_id or evidence_id is required");
    }
    const repo = new PgSourcePostProcessingRepository(this.db);
  ...
  private async executeRule(
    ...
    if (batch.items.length === 0 && batch.evidence.length === 0 && triggerConfig.skip_when_no_new_items) {
      ...
      return repo.markRunFinished({
        ...
        spaceId: rule.space_id,
        status: "skipped",
        summary: "No new intake items matched this rule.",
      });
    }
    return this.executeBatch({
      rule,
  ...
  private async prefilterCandidateBatch(input: {
    ...
    try {
      ...
      const promptBatch: SourcePostProcessingInputBatch = {
        items: promptItems,
        evidence: input.batch.evidence.filter((row) => !row.intake_item_id || promptIds.has(row.intake_item_id)),
  ...
  private async createAndExecuteAgentRun(input: {
    ...
    await refreshIntakePostProcessingAgentPrompt(pool, input.spaceId, input.agentId);
  ...
  private async enqueueDeepAnalysisFollowUp(input: {
    ...
    if (input.itemIds.length === 0) throw new HttpError(422, "Deep analysis follow-up requires intake items");
  ...
  private async buildRetrievalContext(input: {
    ...
    const currentRefs = new Set<string>([
      ...input.batch.items.map((item) => refKey("intake_item", item.id)),
  ...
  private async materializeOutputs(input: {
    ...
    if (input.actions.per_item_summary) {
      for (const summary of input.result.item_summaries) {
        const item = itemById.get(summary.intake_item_id);
        if (!item) continue;
        const artifactId = await repo.insertArtifact({
          spaceId: input.connection.space_id,
          runId: input.agentRun.id,
          ...
          metadata: { ...metadata, action: "per_item_summary", intake_item_id: item.id },
        });
    ...
    if (input.actions.extract_evidence) {
      for (const candidate of input.result.evidence_candidates) {
        const item = itemById.get(candidate.intake_item_id);
        if (!item) continue;
        const evidenceId = await repo.insertEvidence({
          spaceId: input.connection.space_id,
          item,
    ...
    if (input.actions.create_proposals) {
      ...
      const proposalId = await repo.insertProposal({
        ...
        payload: {
          ...
          source_refs: input.items.map((item) => ({ type: "intake_item", id: item.id })),
          metadata,
        },
      });
      proposalIds.push(proposalId);
    }
    if (input.actions.mark_items) {
      await repo.applyItemDecisions(input.connection.space_id, input.result.item_decisions);
    }
    await repo.persistItemDecisions({
      ...
      appliedByMarkItems: input.actions.mark_items,
    });
  ...
  private async resolveAgentId(identity: SpaceUserIdentity, requestedAgentId: string | null): Promise<string> {
    ...
    const pool = this.requirePool();
    const agent = await ensureIntakePostProcessingAgent(pool, identity.spaceId);
    return agent.id;
  }
  ...
  private async loadDecisionItem(spaceId: string, itemId: string): Promise<IntakeItemRow | null> {
  ...
  private async resolveOneOffSourceConnection(
    ...
    const itemRows = itemIds.length
      ? await this.db.query<{ connection_id: string | null }>(
          `SELECT connection_id FROM intake_items
    ...
    if (unique.length !== 1) {
      throw new HttpError(422, "One-off intake post-processing requires inputs from exactly one source");
    }
    return unique[0]!;
  }
...
async function ensureIntakePostProcessingAgent(pool: Pool, spaceId: string): Promise<{ id: string }> {
  const existing = await pool.query<{ id: string }>(
    `SELECT id
       FROM agents
      WHERE space_id = $1
        AND agent_kind = 'system_intake_post_processor'
        AND status = 'active'
  ...
  if (existing.rows[0]) {
    await refreshIntakePostProcessingAgentPrompt(pool, spaceId, existing.rows[0].id);
  ...
  if (!provider) {
    throw new HttpError(
      409,
      "Configure a default model provider before creating intake post-processing rules without an agent.",
    );
  }
  ...
  const created = await agents.create({
    ...
    name: "Intake post-processing",
    description: "System-managed reusable agent for source intake summaries and post-processing.",
    visibility: "space_shared",
    systemPrompt: intakePostProcessingAgentSystemPrompt(),
  ...
  await pool.query(
    `UPDATE agents
        SET owner_user_id = NULL,
            agent_kind = 'system_intake_post_processor',
            updated_at = $3
...
async function refreshIntakePostProcessingAgentPrompt(
  pool: Pool,
  spaceId: string,
  agentId: string,
): Promise<void> {
  await pool.query(
    `UPDATE agent_versions av
      ...
      WHERE a.space_id = $1
        AND a.id = $2
        AND a.agent_kind = 'system_intake_post_processor'
        AND a.current_version_id = av.id
        AND av.system_prompt IS DISTINCT FROM $3`,
    [spaceId, agentId, intakePostProcessingAgentSystemPrompt()],
  );
}

function intakePostProcessingAgentSystemPrompt(): string {
  return [
    "You are the system-managed Intake post-processing agent.",
    ...
    "Do not claim to have read content that is not present in the provided intake items, evidence, retrieval context, or extracted text snippets.",
  ].join("\n");
}
...
async function firstSpaceUserId(pool: Pool, spaceId: string): Promise<string> {
  ...
  if (!fallback) throw new HttpError(409, "Cannot create default intake post-processing agent without an active user");
  return fallback;
}

function promptForActions(actions: SourcePostProcessingActions): string {
  if (actions.per_item_summary && !actions.batch_digest) return "Summarize each intake item.";
  if (actions.extract_evidence) return "Summarize this source material and highlight citable evidence.";
  return "Create a concise digest for these intake items.";
}
...
function mergeSyntheticItemDecisions(
  ...
  const seen = new Set(parsed.map((decision) => decision.intake_item_id));
  return [
    ...parsed,
    ...synthetic.filter((decision) => !seen.has(decision.intake_item_id)),
  ];
}
...
function rankCurrentBatchCandidates(
  ...
  for (const row of batch.evidence) {
    if (!row.intake_item_id) continue;
    const current = evidenceByItem.get(row.intake_item_id) ?? [];
    current.push(joinText([row.title, row.content_excerpt, boundedJsonChars(row.metadata_json, 600)]));
    evidenceByItem.set(row.intake_item_id, current);
  }
...
function fitBatchToPromptBudget(
  ...
  return {
    items: kept,
    evidence: batch.evidence.filter((row) => row.intake_item_id ? keptIds.has(row.intake_item_id) : true),
    cursorBefore: batch.cursorBefore,
    cursorAfter: {
      id: last.id,
...
function estimateItemPromptChars(item: IntakeItemRow, inputConfig: SourcePostProcessingInputConfig): number {
  const metadata = boundedJsonChars(item.metadata_json, 1_200);
...
function retrievalContextDomainConfig(domain: SourcePostProcessingRetrievalDomain): RetrievalContextDomainConfig {
  ...
  if (domain === "intake") {
    return {
      ...
      surface: "source_post_processing_retrieval_context_intake",
    };
  }
  return {
