...
import { fetchIntakeSource } from "../sourceFetch";
...
export class SourcePresetService {
  ...
  async previewArxiv(
    ...
    const response = await fetchIntakeSource(queryUrl, { maxDownloadBytes });
    if (!response.ok) {
  ...
  async createArxiv(identity: SpaceUserIdentity, body: Record<string, unknown>) {
    ...
    return new PgIntakeRepository(this.db, this.config).createConnection(identity, {
      connector_key: "arxiv",
