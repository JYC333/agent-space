...
export function registerSourceRecipeRoutes(app: FastifyInstance, context: ModuleContext): void {
  ...
  app.get("/api/v1/intake/source-recipes/primitives", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    return reply.send({ primitives: listSourceRecipePrimitives() });
  });

  app.post("/api/v1/intake/source-recipes/plan", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const gate = await enforceIntake(context, identity, "intake.source_recipe_create", "source_connection");
  ...
  app.post("/api/v1/intake/source-recipes", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const gate = await enforceIntake(context, identity, "intake.source_recipe_create", "source_connection");
  ...
  app.post("/api/v1/intake/source-recipes/:connectionId/activate", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const connectionId = params(request).connectionId ?? "";
      const gate = await enforceIntake(context, identity, "intake.source_recipe_activate", "source_connection", connectionId);
      if (gate.blocked) return reply.code(403).send(gate.reply403);
  ...
  app.post("/api/v1/intake/custom-sources/:connectionId/bridge-pipeline", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const connectionId = params(request).connectionId ?? "";
      const gate = await enforceIntake(context, identity, "intake.source_recipe_create", "source_connection", connectionId);
      if (gate.blocked) return reply.code(403).send(gate.reply403);
  ...
  app.post("/api/v1/intake/source-recipes/:connectionId/dry-run", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
      const connectionId = params(request).connectionId ?? "";
      const gate = await enforceIntake(context, identity, "intake.source_recipe_dry_run", "source_connection", connectionId);
      if (gate.blocked) return reply.code(403).send(gate.reply403);
  ...
  app.get("/api/v1/intake/connections/:connectionId/recipe-versions", async (request, reply) => {
    const identity = await resolveIdentity(context.config, request, reply);
    if (!identity) return reply;
    try {
  ...
  app.get(
    "/api/v1/intake/connections/:connectionId/recipe-versions/:versionId",
    async (request, reply) => {
      const identity = await resolveIdentity(context.config, request, reply);
      if (!identity) return reply;
      try {
