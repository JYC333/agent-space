...
const INDEXABLE_ITEM_STATUSES = ["new", "triaged", "selected"] as const;
const INDEXABLE_EVIDENCE_STATUSES = ["candidate", "active"] as const;

interface IntakeItemProjectionRow {
  id: string;
...
/**
 * Intake domain adapter for the shared retrieval engine.
 *
 * It projects only lightweight intake metadata and excerpts. Full snapshots and
 * raw extracted artifacts are intentionally not indexed by default; they remain
 * behind explicit reader/source policies and future opt-in settings.
 */
export const intakeRetrievalAdapter: RetrievalDomainAdapter = {
  objectTypes: INTAKE_RETRIEVAL_OBJECT_TYPES,

  async loadCanonical(db, spaceId, objectType, objectId): Promise<CanonicalObject | null> {
    if (objectType === "intake_item") return loadIntakeItem(db, spaceId, objectId);
  ...
  async revalidate(db, spaceId, objectType, objectId): Promise<RevalidatedObject | null> {
    return (await revalidateIntakeMany(db, spaceId, objectType, [objectId])).get(objectId) ?? null;
  },

  async revalidateMany(db, spaceId, objectType, objectIds): Promise<Map<string, RevalidatedObject>> {
    return revalidateIntakeMany(db, spaceId, objectType, objectIds);
  },

  async projectEdges(db, spaceId, object): Promise<RetrievalEdge[]> {
    if (object.objectType === "intake_item") {
      return intakeItemEvidenceEdges(db, spaceId, object.objectId);
    }
    if (object.objectType === "extracted_evidence") {
      return evidenceIntakeItemEdge(db, spaceId, object.objectId);
    }
    return [];
  },
  ...
  async listObjectIds(db, spaceId): Promise<RetrievalObjectRef[]> {
    ...
    const items = await db.query<{ id: string }>(
      ...
      [spaceId, [...INDEXABLE_ITEM_STATUSES]],
    );
    for (const row of items.rows) refs.push({ objectType: "intake_item" as RetrievalObjectType, objectId: row.id });
    ...
    const evidence = await db.query<{ id: string }>(
      `SELECT ee.id
         ...
         JOIN intake_items ii
          ...
          AND ii.status = ANY($2::varchar[])
        WHERE ee.space_id = $1
          AND ee.status = ANY($3::varchar[])
          AND ee.deleted_at IS NULL`,
      [spaceId, [...INDEXABLE_ITEM_STATUSES], [...INDEXABLE_EVIDENCE_STATUSES]],
    );
...
export const intakeRetrievalRegistry = new RetrievalRegistry();
intakeRetrievalRegistry.register(intakeRetrievalAdapter);

async function loadIntakeItem(
  db: Parameters<RetrievalDomainAdapter["loadCanonical"]>[0],
  spaceId: string,
  itemId: string,
): Promise<CanonicalObject | null> {
  const result = await db.query<IntakeItemProjectionRow>(
    `SELECT id, connection_id, item_type, title, source_uri, canonical_uri,
            ...
            status, read_status, content_state, retention_policy, metadata_json,
  ...
  if (!row || !isIndexableItemStatus(row.status)) return null;
  return {
  ...
  const result = await db.query<EvidenceProjectionRow>(
    evidenceSelectSql(
      `ee.space_id = $1
        ...
        AND ii.deleted_at IS NULL
        AND ii.status = ANY($4::varchar[])`,
    ),
    [spaceId, evidenceId, [...INDEXABLE_EVIDENCE_STATUSES], [...INDEXABLE_ITEM_STATUSES]],
  );
  const row = result.rows[0];
  if (!row) return null;
  return {
...
async function revalidateIntakeMany(
    ...
    const result = await db.query<IntakeItemProjectionRow>(
      `SELECT id, connection_id, item_type, title, source_uri, canonical_uri,
        ...
        WHERE space_id = $1
          ...
          AND deleted_at IS NULL`,
      [spaceId, ids, [...INDEXABLE_ITEM_STATUSES]],
    );
    return new Map(result.rows.map((row) => [row.id, { title: row.title, text: intakeItemText(row) }]));
  }
  if (objectType === "extracted_evidence") {
    const result = await db.query<EvidenceProjectionRow>(
      ...
      [spaceId, ids, [...INDEXABLE_EVIDENCE_STATUSES], [...INDEXABLE_ITEM_STATUSES]],
    );
...
async function intakeItemEvidenceEdges(
  db: Queryable,
  spaceId: string,
  itemId: string,
  ...
  return result.rows.map((row) => ({
    from: { objectType: "intake_item", objectId: itemId },
...
async function evidenceIntakeItemEdge(
  db: Queryable,
  spaceId: string,
  evidenceId: string,
): Promise<RetrievalEdge[]> {
  const result = await db.query<EvidenceEdgeRow>(
    `SELECT ee.intake_item_id
       FROM extracted_evidence ee
       JOIN intake_items ii
         ON ii.space_id = ee.space_id
        AND ii.id = ee.intake_item_id
        AND ii.deleted_at IS NULL
        AND ii.status = ANY($3::varchar[])
      WHERE ee.space_id = $1
        AND ee.id = $2
        AND ee.status = ANY($4::varchar[])
        AND ee.deleted_at IS NULL`,
    [spaceId, evidenceId, [...INDEXABLE_ITEM_STATUSES], [...INDEXABLE_EVIDENCE_STATUSES]],
  );
  const intakeItemId = result.rows[0]?.intake_item_id;
  if (!intakeItemId) return [];
  return [{
    from: { objectType: "extracted_evidence", objectId: evidenceId },
    to: { objectType: "intake_item", objectId: intakeItemId },
    relationType: "evidence_for",
    edgeOrigin: "intake",
    edgeStatus: "derived",
    confidence: 0.9,
    evidence: { intake_item_id: intakeItemId },
  }];
}

function evidenceSelectSql(whereClause: string): string {
  return `SELECT ee.id, ee.intake_item_id, ss.connection_id AS source_snapshot_connection_id,
...
function intakeItemText(row: IntakeItemProjectionRow): string {
  return joinText([
    `Title: ${row.title}`,
...
function isIndexableItemStatus(status: string): boolean {
  return (INDEXABLE_ITEM_STATUSES as readonly string[]).includes(status);
}
