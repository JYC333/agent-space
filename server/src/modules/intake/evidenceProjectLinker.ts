...
/**
 * Auto-link an intake item's extracted evidence to every project bound to the
...
export async function linkEvidenceToBoundProjects(
  db: Queryable,
  input: { spaceId: string; intakeItemId: string },
): Promise<number> {
  ...
  const result = await db.query(
    `INSERT INTO evidence_links (
     ...
     ON CONFLICT (space_id, evidence_id, target_type, target_id, link_type)
       WHERE status = 'active'
     DO NOTHING`,
    [input.spaceId, input.intakeItemId, now],
  );
