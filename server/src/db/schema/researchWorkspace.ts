...
export const researchNotebookSections = pgTable("research_notebook_sections", {
  id: varchar({ length: 36 }).primaryKey().notNull(), notebookId: varchar("notebook_id", { length: 36 }).notNull(),
  ...
  index("ix_research_notebook_sections_notebook").on(table.notebookId),
  foreignKey({ columns: [table.notebookId], foreignColumns: [researchNotebooks.id], name: "research_notebook_sections_notebook_fkey" }).onDelete("cascade"),
  ...
  foreignKey({ columns: [table.updatedByRunId], foreignColumns: [runs.id], name: "research_notebook_sections_run_fkey" }).onDelete("set null"),
...
export const researchNotebookSectionRevisions = pgTable("research_notebook_section_revisions", {
  id: varchar({ length: 36 }).primaryKey().notNull(), sectionId: varchar("section_id", { length: 36 }).notNull(),
  ...
  index("ix_research_notebook_section_revisions_section").on(table.sectionId, table.version),
  foreignKey({ columns: [table.sectionId], foreignColumns: [researchNotebookSections.id], name: "research_notebook_section_revisions_section_fkey" }).onDelete("cascade"),
  ...
  foreignKey({ columns: [table.createdByRunId], foreignColumns: [runs.id], name: "research_notebook_section_revisions_run_fkey" }).onDelete("set null"),
...
export const researchIntegrityAlerts = pgTable("research_integrity_alerts", {
  ...
  foreignKey({ columns: [table.sourceItemId], foreignColumns: [sourceItems.id], name: "research_integrity_alerts_source_item_fkey" }).onDelete("set null"),
...
export const researchPaperCards = pgTable("research_paper_cards", {
  ...
  unique("uq_research_paper_cards_project_source").on(table.projectId, table.sourceItemId),
  ...
  foreignKey({ columns: [table.sourceItemId], foreignColumns: [sourceItems.id], name: "research_paper_cards_source_item_fkey" }).onDelete("cascade"),
  foreignKey({ columns: [table.objectId], foreignColumns: [spaceObjects.id], name: "research_paper_cards_object_fkey" }).onDelete("set null"),
...
export const researchChecklistItems = pgTable("research_checklist_items", {
  ...
  foreignKey({ columns: [table.originRunId], foreignColumns: [runs.id], name: "research_checklist_items_run_fkey" }).onDelete("set null"),
