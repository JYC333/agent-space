...
import {
  ContextReviewCycleRequestSchema,
  ContextReviewCycleResponseSchema,
  ContextOpsDrilldownSchema,
  ContextOpsSummarySchema,
} from "../src/contextOps";

describe("context ops contracts", () => {
  it("parses Context Review Cycle request and response contracts", () => {
    const request = ContextReviewCycleRequestSchema.parse({});
    expect(request.create_packets).toBe(true);
    expect(request.review_scope).toBe("private");

    const response = ContextReviewCycleResponseSchema.parse({
  ...
  it("parses aggregate context ops summaries", () => {
    const parsed = ContextOpsSummarySchema.parse({
  ...
  it("parses object and source drill-downs and rejects unknown sections", () => {
    const objects = ContextOpsDrilldownSchema.parse({
    ...
    const sources = ContextOpsDrilldownSchema.parse({
      generated_at: "2026-06-26T00:00:00.000Z",
      space_id: "space-1",
    ...
    const explain = ContextOpsDrilldownSchema.parse({
      generated_at: "2026-06-26T00:00:00.000Z",
      space_id: "space-1",
    ...
    expect(() =>
      ContextOpsDrilldownSchema.parse({
        generated_at: "2026-06-26T00:00:00.000Z",
        space_id: "space-1",
        section: "raw_dump",
        limit: 25,
