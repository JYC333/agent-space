...
Template definitions live in ``core/agent_templates/<key>/template.yaml``.
...
def _template_dir() -> Path:
    ...
    backend package (``core/agent_templates``), and the in-image / mounted copy at
    ``<app_root>/agent_templates`` (``/app/agent_templates`` in Docker, since the
    build context is ``core/`` so ``agent_templates`` is bundled next to ``app``).
    ...
    # Source tree: core/backend/app/agents/ -> core/agent_templates
    candidates.append(_HERE.parents[3] / "agent_templates")
    # Bundled/mounted next to the app package: /app/agent_templates
    candidates.append(_HERE.parents[2] / "agent_templates")
    for path in candidates:
        if path.is_dir():
            return path
    return candidates[-1] if candidates else _HERE.parents[3] / "agent_templates"
