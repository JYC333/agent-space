...
from app.schemas import AgentConfigProposalCreate
from tests.support import factories


def test_execution_config_update_appends_new_version_preserves_old(db, cross_space_pair_db):
    ...
    proposal = AgentService(db).create_config_update_proposal(
        ...
        AgentConfigProposalCreate(
            ...
            model_config_json={"model": "claude-test", "max_tokens": 1234},
        ),
        user_id=ua.id,
    )
    ProposalService(db).accept(proposal.id, space_id=a, user_id=ua.id)
    db.refresh(agent)
...
def test_existing_run_keeps_original_agent_version_pointer(db, cross_space_pair_db):
    a = cross_space_pair_db["space_a_id"]
    ua = cross_space_pair_db["user_a"]
    ...
    proposal = AgentService(db).create_config_update_proposal(
        ...
        AgentConfigProposalCreate(
            base_version_id=v1_id,
            model_config_json={"model": "new", "max_tokens": 99},
        ),
        user_id=ua.id,
    )
    ProposalService(db).accept(proposal.id, space_id=a, user_id=ua.id)
