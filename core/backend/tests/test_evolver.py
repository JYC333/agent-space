...
def _make_memory(db, **kwargs) -> Memory:
...
def test_recency_decay_fresh_memory():
    ...
    m = Memory(scope="user", confidence=1.0, importance=0.5, access_count=0,
...
def test_recency_decay_old_memory():
    old = _utcnow() - timedelta(days=365)
    m = Memory(scope="user", confidence=1.0, importance=0.5, access_count=0,
               created_at=old, last_accessed_at=None)
    decay = _recency_decay(m, _utcnow())
    assert decay < 0.01  # very old, nearly zero


def test_recency_decay_system_scope_never_decays():
    old = _utcnow() - timedelta(days=3650)
    m = Memory(scope="system", confidence=1.0, importance=0.5, access_count=0,
               created_at=old, last_accessed_at=None)
    decay = _recency_decay(m, _utcnow())
...
def test_recency_decay_uses_last_accessed_if_set():
    ...
    m = Memory(scope="user", confidence=1.0, importance=0.5, access_count=5,
...
def test_fitness_fresh_important_memory_is_high():
    m = Memory(scope="user", importance=1.0, confidence=1.0, access_count=10,
               created_at=_utcnow(), last_accessed_at=None)
    score = _fitness(m, _utcnow())
    assert score > 0.8


def test_fitness_old_unaccessed_memory_is_low():
    old = _utcnow() - timedelta(days=500)
    m = Memory(scope="user", importance=0.5, confidence=0.5, access_count=0,
               created_at=old, last_accessed_at=None)
    score = _fitness(m, _utcnow())
...
def test_fitness_zero_importance_treated_as_default():
    ...
    m = Memory(scope="user", importance=0.0, confidence=1.0, access_count=0,
               created_at=_utcnow(), last_accessed_at=None)
    score = _fitness(m, _utcnow())
...
def test_decay_and_archive_dry_run_does_not_change_status(db):
    old = _utcnow() - timedelta(days=500)
    m = _make_memory(db, title="Old memory", importance=0.01, confidence=0.1, created_at=old)
...
def test_decay_and_archive_live_run_archives_low_fitness(db):
    old = _utcnow() - timedelta(days=500)
    m = _make_memory(db, title="Stale", importance=0.01, confidence=0.05, created_at=old)
...
def test_decay_and_archive_never_archives_system_scope(db):
    old = _utcnow() - timedelta(days=500)
    m = _make_memory(db, scope="system", title="System rule", importance=0.01,
                     confidence=0.01, created_at=old)
