...
LOCAL_DEPLOY = REPO_ROOT / "deployments" / "local"
START_SH = REPO_ROOT / "scripts" / "start.sh"
LOCAL_COMPOSE_HELPER = REPO_ROOT / "scripts" / "lib" / "local-compose.sh"
LOCAL_COMPOSE_SCRIPTS = [
    START_SH,
    REPO_ROOT / "scripts" / "db" / "migrate.sh",
    REPO_ROOT / "scripts" / "db" / "reset-postgres.sh",
    REPO_ROOT / "scripts" / "db" / "dump.sh",
    REPO_ROOT / "scripts" / "db" / "restore.sh",
    REPO_ROOT / "scripts" / "db" / "shell.sh",
    REPO_ROOT / "scripts" / "system" / "backup.sh",
    REPO_ROOT / "scripts" / "system" / "restore.sh",
    REPO_ROOT / "scripts" / "system" / "verify-restore.sh",
]
...
def test_mode_specific_env_templates_exist():
    for mode in ("dev", "test", "prod"):
        assert (LOCAL_DEPLOY / f".env.{mode}.example").is_file()


def test_dev_and_test_templates_keep_local_postgres_defaults():
    for mode in ("dev", "test"):
        text = (LOCAL_DEPLOY / f".env.{mode}.example").read_text()
...
def test_prod_template_does_not_contain_development_postgres_password():
    text = (LOCAL_DEPLOY / ".env.prod.example").read_text()
...
def test_start_prod_rejects_development_and_placeholder_passwords():
    ...
    assert 'ENV_TEMPLATE="$COMPOSE_DIR/.env.$MODE.example"' in text
