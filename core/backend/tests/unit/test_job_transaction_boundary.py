...
def _install_handler(job_type: str, fn):
    old = handlers._REGISTRY.get(job_type)
    handlers._REGISTRY[job_type] = fn
    return old


def _restore_handler(job_type: str, old) -> None:
    if old is None:
        handlers._REGISTRY.pop(job_type, None)
    else:
        handlers._REGISTRY[job_type] = old


async def _run_job_inline(job, queue):
    ...
    try:
        await _run_job(job, queue, asyncio.Semaphore(1))
    finally:
...
def test_terminal_success_survives_auxiliary_event_write_failure():
    ...
    old = _install_handler(job_type, lambda job: {"ok": True})
    try:
        asyncio.run(_run_job_inline(_job(job_type), queue))
    finally:
        _restore_handler(job_type, old)
...
def test_terminal_failure_survives_auxiliary_event_write_failure():
    ...
    old = _install_handler(job_type, handler)
    try:
        asyncio.run(_run_job_inline(_job(job_type), queue))
    finally:
        _restore_handler(job_type, old)
