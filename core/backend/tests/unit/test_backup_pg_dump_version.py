...
DOCKERFILE = REPO_ROOT / "core" / "backend" / "Dockerfile"
ENV_EXAMPLES = [
    REPO_ROOT / "deployments" / "local" / ".env.dev.example",
    REPO_ROOT / "deployments" / "local" / ".env.test.example",
    REPO_ROOT / "deployments" / "local" / ".env.prod.example",
]
COMPOSE_FILES = [
    REPO_ROOT / "deployments" / "local" / "docker-compose.dev.yml",
    REPO_ROOT / "deployments" / "local" / "docker-compose.test.yml",
    REPO_ROOT / "deployments" / "local" / "docker-compose.prod.yml",
]
...
def test_compose_uses_stable_postgres_container_names():
    for mode in ("dev", "test", "prod"):
        text = (REPO_ROOT / "deployments" / "local" / f"docker-compose.{mode}.yml").read_text()
...
def test_test_compose_keeps_backend_internal_port_8000_with_host_8100():
    text = (REPO_ROOT / "deployments" / "local" / "docker-compose.test.yml").read_text()
    assert '- "8100:8000"' in text
    assert "command: uvicorn app.main:app --host 0.0.0.0 --port 8000" in text
    assert "API_URL=http://backend:8000" in text
