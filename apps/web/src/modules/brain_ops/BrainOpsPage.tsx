...
import { brainOpsApi, knowledgeApi, objectSchemaApi } from '../../api/client'
...
function countEntries(counts: BrainOpsCountMap): Array<[string, number]> {
  return Object.entries(counts).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
}

function total(counts: BrainOpsCountMap): number {
...
function CountList({ counts, empty = 'None' }: { counts: BrainOpsCountMap; empty?: string }) {
...
const ARTIFACT_SECTIONS: BrainOpsDrilldownSection[] = ['maintenance_reports', 'diagnostics_reports', 'explain_reports', 'recent_briefs']
...
function Drilldown({
  ...
  const [data, setData] = useState<BrainOpsDrilldown | null>(null)
  ...
  const toggle = useCallback(async () => {
    ...
    if (next && !data) {
      setLoading(true)
      try {
        setData(await brainOpsApi.drilldown(section, { limit: 25 }))
      } catch (error) {
...
export default function BrainOpsPage() {
  ...
  const [summary, setSummary] = useState<BrainOpsSummary | null>(null)
  const [loading, setLoading] = useState(true)
  const [permissionDenied, setPermissionDenied] = useState(false)
  const [scanBusy, setScanBusy] = useState<null | 'maintenance' | 'diagnostics' | 'dream' | 'contradictions' | 'discovery' | 'shape'>(null)
  ...
  useEffect(() => {
    const key = `brain_ops.explain_presets.${activeSpaceId ?? 'default'}`
    try {
  ...
  const load = useCallback(async () => {
    ...
    setPermissionDenied(false)
    try {
      setSummary(await brainOpsApi.summary({ window_days: windowDays, limit: 10 }))
    } catch (error) {
  ...
  const runDreamCycle = useCallback(async () => {
    setScanBusy('dream')
    try {
      const result = await brainOpsApi.dreamCycleV2({
        window_days: windowDays,
        create_packets: true,
      ...
      toast.success(
        result.degraded
          ? `Dream Cycle complete with ${result.warnings.length} warning(s).`
          : result.claim_candidates.proposal_id
          ? 'Dream Cycle complete — review packets created.'
  ...
  const createClaimPacket = useCallback(async (
    ...
    if (promotePrivate) {
      const confirmed = window.confirm(
        'Promote private-source derived claim candidates into shared Brain Ops review for this space?',
      )
      if (!confirmed) return
    }
  ...
  const saveExplainPreset = useCallback(() => {
    ...
    localStorage.setItem(`brain_ops.explain_presets.${activeSpaceId}`, JSON.stringify(next))
    ...
    <div className="space-y-5 p-6">
      <div className="flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Brain Ops</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={runDreamCycle} disabled={scanBusy !== null || !activeSpaceId || permissionDenied}>
            {scanBusy === 'dream' ? <Loader2 className="size-4 animate-spin" /> : <Play className="size-4" />}
            Run Dream Cycle
          </Button>
        </div>
      </div>
      ...
      {!activeSpaceId ? (
        <Card className="p-4 text-sm text-muted-foreground">Select an operational space to inspect Brain Ops.</Card>
      ) : waitingForSpace ? (
        ...
        <Card className="p-4">
          <CardTitle className="text-sm">Brain Ops access is not enabled</CardTitle>
          <p className="mt-2 text-sm text-muted-foreground">
            This space only exposes Brain Ops to owners and admins unless member review or member scan initiation is enabled in retrieval settings.
          </p>
        </Card>
      ) : loading && !summary ? (
        <div className="grid gap-4 md:grid-cols-2">
        ...
        <>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            <Panel icon={BrainCircuit} title="Brain Loops">
            ...
            <Panel icon={BrainCircuit} title="Memory Provenance">
              <div className="flex flex-wrap gap-2">
