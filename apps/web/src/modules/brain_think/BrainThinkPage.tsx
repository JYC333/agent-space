...
import { brainThinkApi, knowledgeApi } from '../../api/client'
...
const DOMAIN_LABELS: Record<BrainThinkDomain, string> = {
  knowledge: 'Knowledge',
  memory: 'Memory',
  project: 'Project summaries',
}

const ALL_DOMAINS: BrainThinkDomain[] = ['knowledge', 'memory', 'project']

function GapBadges({ summary }: { summary: BrainThinkResponse['gap_summary'] }) {
...
function ReviewableGaps({ gap }: { gap: NonNullable<BrainThinkDomainSection['brief']>['gap_analysis'] }) {
  const sourceGaps = [
...
function DomainSection({ section }: { section: BrainThinkDomainSection }) {
...
export default function BrainThinkPage() {
  const { activeSpaceId } = useSpace()
  const [query, setQuery] = useState('')
  const [domains, setDomains] = useState<BrainThinkDomain[]>(['knowledge'])
  ...
  const [result, setResult] = useState<BrainThinkResponse | null>(null)

  const toggleDomain = useCallback((domain: BrainThinkDomain) => {
  ...
  const ask = useCallback(async () => {
    ...
    if (!trimmed) {
      toast.error('Enter a question to ask the brain.')
      return
    }
    if (domains.length === 0) {
    ...
    try {
      ...
      setResult(await brainThinkApi.think({
        query: trimmed,
        domains: ordered,
        persist,
        combine,
  ...
  const runFollowUp = useCallback(async (followUp: BrainThinkFollowUp) => {
  ...
  const provenanceByDomain = useMemo(() => {
    const grouped = new Map<BrainThinkDomain, BrainThinkResponse['provenance']>()
  ...
  return (
    <div className="space-y-5 p-6">
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Ask Brain</h1>
      ...
      {!activeSpaceId ? (
        <Card className="p-4 text-sm text-muted-foreground">Select a space to ask the brain.</Card>
      ) : (
        <>
          <Card className="p-4">
            <Textarea
