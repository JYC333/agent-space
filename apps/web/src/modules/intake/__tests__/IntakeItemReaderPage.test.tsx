...
import IntakeItemReaderPage from '../IntakeItemReaderPage'
...
const mockedApi = vi.mocked(intakeReaderApi)
const mockedIntakeApi = vi.mocked(intakeApi)

const docPayload: ReaderDocumentPayload = {
  document_type: 'intake_item',
  document_id: 'item-1',
  space_id: 'space-1',
...
async function renderPage() {
  render(
    <MemoryRouter initialEntries={['/intake/items/item-1/reader']}>
      <Routes>
        <Route path="/intake/items/:itemId/reader" element={<IntakeItemReaderPage />} />
      </Routes>
    </MemoryRouter>,
  )
...
describe('IntakeItemReaderPage', () => {
  it('loads the reader document and active annotations', async () => {
    await renderPage()

    expect(mockedApi.getDocument).toHaveBeenCalledWith('intake_item', 'item-1')
    expect(mockedApi.listAnnotations).toHaveBeenCalledWith('intake_item', 'item-1')
  ...
  it('creates a highlight from the toolbar with the document target and anchor payload', async () => {
    ...
    expect(mockedApi.createAnnotation).toHaveBeenCalledWith(expect.objectContaining({
      ...
      anchor_json: expect.objectContaining({
        ...
        block_ref: { index: 1, node_type: 'paragraph', from: 18, to: 37 },
        content_hash: 'hash-1',
        document_ref: { document_type: 'intake_item', document_id: 'item-1' },
      }),
    }))
  ...
  it('re-extracts the current item from the reader header and reloads the document', async () => {
    ...
    mockedIntakeApi.itemAction.mockResolvedValue({} as never)
    mockedIntakeApi.jobs.mockResolvedValue({
      items: [{
        id: 'extract-job-1',
        space_id: 'space-1',
        connection_id: null,
    ...
    mockedIntakeApi.runJob.mockResolvedValue({ status: 'succeeded' } as never)
    await renderPage()

    await userEvent.click(screen.getByRole('button', { name: 'Re-extract' }))

    await waitFor(() => {
      expect(mockedIntakeApi.itemAction).toHaveBeenCalledWith('item-1', 'queue_content')
      expect(mockedIntakeApi.jobs).toHaveBeenCalledWith({
        intake_item_id: 'item-1',
        job_type: 'extract_text',
        status: 'pending',
        limit: 1,
      })
      expect(mockedIntakeApi.runJob).toHaveBeenCalledWith('extract-job-1')
