...
import { ArrowLeft, ExternalLink, FileText, PanelRight, RefreshCw } from 'lucide-react'

type Visibility = 'private' | 'space_shared'

export default function IntakeItemReaderPage() {
    ...
    ;(async () => {
      ...
      try {
        const [d, annRes] = await Promise.all([
          intakeReaderApi.getDocument('intake_item', itemId),
          intakeReaderApi.listAnnotations('intake_item', itemId),
        ])
        if (!cancelled) {
          setDoc(d)
  ...
  const selectAnnotation = useCallback((ann: ReaderAnnotation, opts?: { scroll?: boolean }) => {
    ...
    intakeReaderApi.listThreads(ann.id)
  ...
  const createAnnotation = useCallback(async (type: ReaderAnnotationType, sel: TextSelection) => {
    ...
    try {
      ...
      if (doc.document_type === 'intake_item') body.intake_item_id = doc.document_id
      ...
      const created = await intakeReaderApi.createAnnotation(body)
  ...
  async function reextractReaderDocument() {
    if (!doc || doc.document_type !== 'intake_item' || !doc.content_state) return
    ...
    try {
      await intakeApi.itemAction(doc.document_id, 'queue_content')
      await runQueuedItemJob(doc.document_id, 'extract_text', 'Text extraction')
      const [d, annRes] = await Promise.all([
        intakeReaderApi.getDocument('intake_item', doc.document_id),
        intakeReaderApi.listAnnotations('intake_item', doc.document_id),
      ])
      setDoc(d)
      setAnnotations(annRes.items)
      setSelection(null)
    ...
    return (
      <div className="p-6">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/intake/items/${itemId}`}><ArrowLeft className="size-4 mr-1" />Item</Link>
        </Button>
        <EmptyState
  ...
  const readerTextExtractionReason = doc.document_type === 'intake_item' && doc.content_state
    ? textExtractionDisabledReason({ content_state: doc.content_state, source_uri: doc.source_uri })
    : 'Text extraction is only available for intake items.'
