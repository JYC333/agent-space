...
- Workspace/agent digests are **shared-only**: `loadScopeMemories` filters
  `visibility = 'space_shared'` and applies workspace scope separately. It
  excludes `highly_restricted`; private and selected-user memory never enters a
  cache-shared bundle.
