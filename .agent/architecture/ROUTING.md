...
reasons, fallback chain, and hint sources per physical attempt.
`runs.route_decision_id` stamps the current run route. Historical verification rates use only runs with verification results in
