...
is only permitted in:
  - backend/app/policy/**  (the policy package itself)
  - backend/app/runs/preflight.py  (dry-run simulation, no side effects)
  - backend/app/runs/run_service.py  (non-mutating preflight in create_run,
      no PolicyDecisionRecord — real enforcement in RunExecutionService)
  - backend/app/agents/agent_service.py  (non-mutating preflight before
      queuing a run — no PolicyDecisionRecord — real enforcement in RunExecutionService)
  - backend/app/automation/policy_preflight.py  (non-mutating automation
