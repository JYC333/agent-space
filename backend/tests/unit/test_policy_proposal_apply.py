...
def test_check_policy_registered_type_without_risk_table_entry_denies(db, cross_space_pair_db):
    """A registered applier is not enough: proposal.apply also requires an
    explicit policy risk-table entry so new proposal types fail closed."""
    from app.proposals import (
        ProposalApplierRegistry,
        ProposalApplyResult,
        get_proposal_applier_registry,
    )
    from app.proposals.applier_registry import init_registry
    ...
    try:
        ...
        registry.register("workspace_profile_update", lambda ctx: ProposalApplyResult(proposal=ctx.proposal))
        ...
        decision = check_proposal_apply_policy(
            ...
            user_id=ua.id,
            space_id=a,
            proposal=_FakeProposal("workspace_profile_update", None),
        )
    finally:
