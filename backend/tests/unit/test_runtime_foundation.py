...
class TestProcessRegistry:
    ...
    def test_local_executor_registers_and_deregisters(self, tmp_path):
        """LocalExecutor registers the subprocess PID through the injected
        runs-owned bridge and deregisters after completion."""
        ...
        result = executor.run_command(
            ...
            timeout=10,
            run_id=run_id,
            process_registry=RunProcessRegistryAdapter(),
        )
    ...
    def test_local_executor_without_registry_does_not_track(self):
        """Without an injected registry the subprocess runs unregistered —
        there is no fallback into app.runs."""
        from app.runtimes.local_executor import LocalExecutor
        from app.runs.process_registry import get_pid

        run_id = "proc-exec-test-no-registry"
