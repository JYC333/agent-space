...
The default space is the owner's personal space, created with a generated UUID
and located by owner membership — there is no fixed/magic space id.
"""
...
from app.spaces.defaults import personal_space_id_for_owner
...
def test_bootstrap_creates_usable_initial_state(empty_engine):
    ...
    with Session() as db:
        created = bootstrap_instance(db, user_id=USER_ID)
    ...
    assert created == {
        ...
        "note_collections": True,
        "system_templates": True,
    }

    with Session() as db:
        space_id = personal_space_id_for_owner(db, USER_ID)
        assert space_id is not None, "bootstrap did not create the owner's personal space"

        space = db.query(Space).filter(Space.id == space_id).one()
        assert space.type == "personal"
        # The space id is a generated UUID, not a magic string.
        assert space.id != "personal"
...
def test_bootstrap_is_idempotent(empty_engine):
    Session = sessionmaker(bind=empty_engine)
    with Session() as db:
        bootstrap_instance(db, user_id=USER_ID)
        space_id = personal_space_id_for_owner(db, USER_ID)
    ...
    with Session() as db:
        # Exactly one personal space for the owner — no duplicate minted.
        assert (
            db.query(Space)
            .filter(Space.created_by_user_id == USER_ID, Space.type == "personal")
            .count()
            == 1
        )
...
def test_bootstrap_summary_reports_execution_planes_only_when_rows_inserted(empty_engine):
    ...
    Session = sessionmaker(bind=empty_engine)
    with Session() as db:
        first = bootstrap_instance(db, user_id=USER_ID)
    assert first["execution_planes"] is True

    with Session() as db:
        second = bootstrap_instance(db, user_id=USER_ID)
...
def test_bootstrap_can_skip_execution_planes(empty_engine):
    Session = sessionmaker(bind=empty_engine)
    with Session() as db:
        created = bootstrap_instance(db, user_id=USER_ID, seed_execution_planes=False)
        space_id = personal_space_id_for_owner(db, USER_ID)
