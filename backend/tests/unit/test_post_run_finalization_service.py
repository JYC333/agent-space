...
class TestBridgeHookFailureSemantics:
    """A failing tasks-owned bridge hook must fail the finalization exactly like
    the old inline bridge: failed RunFinalization with error_json stage
    ``task_evaluation_bridge``, one failed run_finalized event, idempotent.
    """

    def _monkeypatch_bridge_failure(self, monkeypatch) -> None:
        def _fail(*args, **kwargs):
            raise RuntimeError("forced bridge failure")

        monkeypatch.setattr(
            "app.tasks.evaluation_service.TaskEvaluationService.create_from_run_evaluation",
            _fail,
        )

    def test_bridge_failure_records_failed_finalization_with_stage(self, db, monkeypatch):
        space, user = _setup(db)
        run = _terminal_run(db, space_id=space, user_id=user, status="succeeded")
        _task_link(db, space_id=space, user_id=user, run=run)
        self._monkeypatch_bridge_failure(monkeypatch)

        fin = PostRunFinalizationService(db).finalize(run.id, space_id=space)
        db.flush()

        assert fin.status == "failed"
        assert fin.error_json["stage"] == "task_evaluation_bridge"
        assert "forced bridge failure" in fin.error_json["error"]
        assert fin.run_evaluation_id is not None
        assert fin.task_evaluation_id is None

    def test_bridge_failure_emits_single_failed_event_and_is_idempotent(self, db, monkeypatch):
        space, user = _setup(db)
        run = _terminal_run(db, space_id=space, user_id=user, status="succeeded")
        _task_link(db, space_id=space, user_id=user, run=run)
        self._monkeypatch_bridge_failure(monkeypatch)
        ...
        assert len(events) == 1
        assert events[0].status == "failed"
        assert db.query(TaskEvaluation).filter(TaskEvaluation.run_id == run.id).count() == 0
