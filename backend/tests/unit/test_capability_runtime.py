...
def _create_external_capability_workspace(
    ...
    *,
    ...
    cap_id: str = "external.sample",
    module_dir: str = "external_sample",
    ...
    _write_capability(
        ...
        main_py=(
            ...
            "    return {\n"
            "        'status': 'succeeded',\n"
            "        'output': {'received_input': data},\n"
            "        'artifacts': [{\n"
            "            'artifact_type': 'external.sample.result.v1',\n"
            "            'title': 'External Sample Result',\n"
            "            'content': 'External sample capability executed.',\n"
            "            'metadata_json': {'received_input': data},\n"
            "        }],\n"
            "    }\n"
        ),
    )
...
def test_external_capability_workspace_discovery_sets_source_metadata(db, tmp_path):
    ...
    cap = registry.get("external.sample")
...
def test_ordinary_workspace_is_not_scanned_for_capabilities(db, tmp_path):
    ...
    assert registry.get("external.sample") is None
...
def test_external_capability_enable_persists_across_registry_reload(db, tmp_path, monkeypatch):
    ...
    registry = CapabilityRegistry(db)
    registry.reload(space_id=space_id)
    assert registry.get("external.sample").enabled is False
    assert registry.set_enabled("external.sample", True) is not None

    reloaded = CapabilityRegistry(db)
    reloaded.reload(space_id=space_id)
    cap = reloaded.get("external.sample")

    assert cap is not None
    assert cap.enabled is True
    assert "external.sample" in load_enabled_external_capabilities(str(instance_root))
...
def test_external_capability_disable_persists_across_registry_reload(db, tmp_path, monkeypatch):
    ...
    registry = CapabilityRegistry(db)
    registry.reload(space_id=space_id)
    registry.set_enabled("external.sample", True)
    registry.set_enabled("external.sample", False)
    ...
    assert cap is not None
    assert cap.enabled is False
    assert "external.sample" not in load_enabled_external_capabilities(str(instance_root))
...
def test_persisted_enabled_external_capability_missing_from_discovery_is_ignored(
    ...
    assert registry.get("external.sample") is not None
    assert registry.get("external.sample").enabled is False
...
def test_newly_discovered_external_capability_is_not_auto_enabled(db, tmp_path, monkeypatch):
    ...
    _create_external_capability_workspace(
        ...
        space_id=space_id,
        user_id=user.id,
        cap_id="external.sample",
        module_dir="external_sample",
    )
    db.commit()
    ...
    workspace_root = tmp_path / "workspace-external_sample_two"
    capability_root = workspace_root / "capabilities"
    _write_capability(
        capability_root,
        "external.sample.two",
        module_dir="external_sample_two",
    )
    factories.create_test_workspace(
        db,
        space_id=space_id,
        root_path=str(workspace_root),
        name="workspace-external_sample_two",
    ...
    assert reloaded.get("external.sample").enabled is True
    assert reloaded.get("external.sample.two") is not None
    assert reloaded.get("external.sample.two").enabled is False
...
def test_external_capability_defaults_disabled_and_fails_execution(
    ...
    run.capability_id = "external.sample"
    db.commit()
...
def test_enabled_external_capability_executes_through_run_service(
    ...
    run.prompt = '{"message": "hello sample"}'
    db.commit()
    ...
    artifact = (
        ...
        .filter(
            ...
            Artifact.artifact_type == "external.sample.result.v1",
        )
        .one()
    )
    assert artifact.project_id == project.id
    assert artifact.title == "External Sample Result"
    assert artifact.metadata_json == {"received_input": {"message": "hello sample"}}
