...
# by ops/scripts/). In the Docker backend container it is the bind mount /aspace;
...
class AppPaths:
    ...
    def validate(self) -> None:
        ...
        operator's responsibility (``ops/scripts/start.sh`` creates mode-700 trees).
...
class Settings(BaseSettings):
    ...
    # explicitly. Scripts such as ops/scripts/db/migrate.sh may read these as fallback
    ...
    capabilities_dir: str = str(Path(__file__).parents[2] / "catalog" / "capabilities")
    ...
    # and via API). ops/scripts/system/backup.sh and ops/scripts/system/restore.sh are
    # the offline full-system equivalents (same archive format) for when the
    # backend is not running. ops/scripts/db/ holds DB-only expert tools.
    #
