...
- Task-level bridging is owned by ``tasks``: it registers a run-finalized hook
  (``task_evaluation_bridge``) on the runs-owned RunFinalizedHookRegistry, so
  this module never imports ``tasks``.
"""
...
from .lifecycle_hooks import (
    ...
    get_registry as get_run_finalized_hook_registry,
)
...
class PostRunFinalizationService:
    ...
    def _do_finalize(self, run: Run, *, space_id: str) -> RunFinalization:
        ...
        # Module-owned post-run side effects (e.g. the tasks-owned
        # RunEvaluation -> TaskEvaluation bridge) run here through the
        # run-finalized hook registry; a failing hook fails this finalization
        # with its hook name as the stage, exactly like the old inline bridge.
        hook_context = RunFinalizedContext(
            db=self.db,
            run_id=run.id,
            space_id=space_id,
        ...
        try:
            get_run_finalized_hook_registry().run(hook_context)
        except RunFinalizedHookFailure as exc:
            original = exc.__cause__ or exc
            log.error(
                "finalization: run-finalized hook %r failed run=%s: %s",
                exc.hook_name, run.id, original,
            )
            return self._record_failed(
                run,
                space_id=space_id,
                run_evaluation_id=run_eval_id,
                error_json={"stage": exc.hook_name, "error": str(original)},
            )
        except Exception as exc:
            log.error("finalization: run-finalized hook dispatch failed run=%s: %s", run.id, exc)
            return self._record_failed(
                run,
                space_id=space_id,
                run_evaluation_id=run_eval_id,
                error_json={"stage": "run_finalized_hooks", "error": str(exc)},
            )

        task_eval_id = hook_context.task_evaluation_id
        skipped_reasons = list(hook_context.skipped_reasons)
