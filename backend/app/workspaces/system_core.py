...
def _ensure_personal_space(db: Session, user: User) -> Space:
    ...
    # Hooks now run inside this caller's transaction (commit=False), so own the
    # commit here: the space + its default seeds must persist atomically even
    # when workspace registration below is later skipped (e.g. deployer offline).
    db.commit()
