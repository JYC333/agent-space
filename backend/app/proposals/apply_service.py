...
Type-specific apply dispatch goes through the proposals-owned
``ProposalApplierRegistry`` (``app.proposals.applier_registry``): each
proposal-owning module registers its appliers via
``register_proposal_appliers(registry)`` wired by
``app.modules.registry.register_proposal_appliers``. This service keeps the
cross-cutting apply orchestration — accept-context guard, grant egress
approval, source monitoring, digest invalidation — and the memory-owned apply
business logic below (MemoryProposalApplier, PolicyProposalApplier,
FollowUpTaskProposalApplier; registered in ``app.memory.proposal_appliers``).
...
# Re-exports preserved for existing callers: ProposalApplyError is the
# proposals-owned apply boundary error; ApplyResult is the proposals-owned
...
class MemoryProposalApplier:
    ...
    def apply_archive(self, proposal: Proposal, *, user_id: str) -> MemoryApplyResult:
        ...
        from ..memory import (
            MemoryInternalWriter,
            TARGET_MEMORY,
            merge_distinct_provenance_entries,
            proposal_provenance_entry,
            provenance_entries_from_payload,
            write_provenance_links,
        )
...
class PolicyProposalApplier:
    ...
    def apply(self, proposal: Proposal, *, user_id: str) -> PolicyApplyResult:
        ...
        payload = proposal.payload_json or {}
        from ..policy import PolicyEffectValidationError, validate_policy_change_payload
...
# ---------------------------------------------------------------------------
# ProposalApplyService — cross-cutting orchestration around registry dispatch
# ---------------------------------------------------------------------------
...
class ProposalApplyService:
    ...
    *before* calling apply().  This service assumes the proposal is already
    ...
    Type-specific writes are dispatched through the proposals-owned
    ``ProposalApplierRegistry``; this service owns only the cross-cutting
    steps that must run for every apply (accept-context guard, grant egress
    approval, source monitoring, digest invalidation).
    ...
    **Source monitoring parameters** — ``bypass_source_monitoring`` and
    ...
    def supported_types() -> frozenset[str]:
        """Proposal types with a registered apply applier."""
        return frozenset(get_proposal_applier_registry().registered_appliers())
    ...
    def _enforce_source_monitoring(
        ...
        *,
        ...
        if bypass_source_monitoring:
            return
        from ..memory import SourceMonitoringService, monitoring_snapshot
    ...
    def apply(
        ...
        *,
        ...
        """
        registry = get_proposal_applier_registry()
        ...
        result = registry.apply(
            ProposalApplyContext(db=self._db, proposal=proposal, user_id=user_id)
        )
