...
def seed_system_memories_for_space(db: Session, space_id: str, *, commit: bool = True) -> int:
    ...
    tests) keep the default and the writer commits each seed; callers that run
    inside a larger transaction (e.g. the space-created hook) pass ``commit=False``
    so the rows flush but the surrounding caller owns the single commit. This
    keeps space creation atomic — a later hook failure rolls back these rows too.
    """
    ...
    for seed in _system_seeds_for_space(space_id):
        ...
        if not exists:
            writer.create_system_seed_memory(seed, created_by="system_seed", commit=commit)
            inserted += 1
    return inserted
