...
export interface DairyEntryRow {
  id: string;
  user_id: string;
  entry_date: string;
  content: string;
...
export interface DairyReflectionRow {
  id: string;
  entry_id: string;
...
export const dairyRepository = {
  async findEntry(db: Queryable, userId: string, date: string): Promise<DairyEntryRow | null> {
    const result = await db.query<DairyEntryRow>(
  ...
  async upsertEntry(db: Queryable, input: UpsertEntryInput): Promise<DairyEntryRow> {
    const result = await db.query<DairyEntryRow>(
      `INSERT INTO dairy_entries (user_id, entry_date, content)
       VALUES ($1, $2::date, $3)
  ...
  async deleteEntry(db: Queryable, userId: string, date: string): Promise<boolean> {
    const result = await db.query(
      `DELETE FROM dairy_entries WHERE user_id = $1 AND entry_date = $2::date`,
      [userId, date],
    );
    return (result.rowCount ?? 0) > 0;
  },

  async findOnThisDay(db: Queryable, userId: string, date: string): Promise<DairyEntryRow[]> {
    const result = await db.query<DairyEntryRow>(
      `SELECT id, user_id, entry_date::text, content, created_at::text, updated_at::text
         FROM dairy_entries
        WHERE user_id = $1
  ...
  async findReflectionsForEntry(db: Queryable, entryId: string): Promise<DairyReflectionRow[]> {
    const result = await db.query<DairyReflectionRow>(
      `SELECT id, entry_id, reflection_date::text, content, ai_model, created_at::text
         FROM dairy_reflections
        WHERE entry_id = $1
  ...
  async insertReflection(
    ...
    content: string,
    aiModel?: string,
  ): Promise<DairyReflectionRow> {
    const result = await db.query<DairyReflectionRow>(
      `INSERT INTO dairy_reflections (entry_id, reflection_date, content, ai_model)
       VALUES ($1, $2::date, $3, $4)
