...
import { DAIRY_PLUGIN_ID } from "./manifest";

const SQL_FILE_RE = /^(\d{4}_.+)\.sql$/;

export const dairyMigrations = loadDairyMigrations();

function loadDairyMigrations(): readonly PluginMigration[] {
  ...
  if (invalid.length > 0) {
    throw new Error(
      `Invalid SQL file name(s) for plugin ${DAIRY_PLUGIN_ID}: ${invalid.join(", ")}`,
    );
  }
  if (sqlFiles.length === 0) {
    throw new Error(`No SQL files found for plugin ${DAIRY_PLUGIN_ID} in ${migrationDir}`);
  }
...
function resolveMigrationDir(): string {
  ...
  throw new Error(
    `Migration directory not found for plugin ${DAIRY_PLUGIN_ID}. Checked: ${candidates.join(", ")}`,
  );
}
