...
import type { DairyApi, DairyEntry, DairyReflection, DairyWebHost } from './host'

export type { DairyApi, DairyEntry, DairyReflection, DairyWebHost } from './host'

const PLUGIN_ID = 'dairy'
...
function OnThisDay({ api, date, currentEntryId }: { api: DairyApi; date: string; currentEntryId?: string }) {
...
function Reflections({ api, date }: { api: DairyApi; date: string }) {
  const [reflections, setReflections] = useState<DairyReflection[]>([])
...
function HistorySidebar({ api, selectedDate, onSelect }: { api: DairyApi; selectedDate: string; onSelect: (date: string) => void }) {
  const [entries, setEntries] = useState<DairyEntry[]>([])
  ...
  if (!enabled) {
    return (
      <div style={{ maxWidth: 520, margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Dairy</h1>
...
export function createDairyPage(host: DairyWebHost): ComponentType {
  return function DairyPageWithHost() {
    return <DairyPage host={host} />
  }
}
