...
// upstream, in RunOrchestrationService.enforceRuntimePolicy via the Python
// policy.enforce port (verified by runOrchestrationService + the contract
// tests). This adapter is only reached after that gate allows the run, so it
// holds no policy seam of its own.
