import { randomUUID } from "node:crypto";
import type { ControlPlaneConfig } from "../../config";
import { getDbPool } from "../../db/pool";
import type { Queryable } from "./repository";
import {
  redactEvidenceText,
  sanitizeEvidenceJson,
} from "./evidenceRedaction";

export type RunJobStatus =
  | "pending"
  | "claimed"
  | "running"
  | "completed"
  | "failed"
  | "cancelled";

export interface RunJobRecord {
  id: string;
  space_id: string;
  user_id: string | null;
  workspace_id: string | null;
  agent_id: string | null;
  job_type: string;
  status: RunJobStatus;
  priority: number;
  payload_json: Record<string, unknown>;
  result_json: Record<string, unknown> | null;
  error: string | null;
  attempts: number;
  max_attempts: number;
  scheduled_at: string;
  claimed_by: string | null;
  claimed_at: string | null;
  started_at: string | null;
  completed_at: string | null;
  heartbeat_at: string | null;
  created_at: string;
  updated_at: string;
}

export interface RunJobEventRecord {
  id: string;
  job_id: string;
  event_type: string;
  message: string;
}

export interface RunJobReclaimResult {
  reclaimed_count: number;
}

const JOB_SELECT_COLUMNS = `
  id, space_id, user_id, workspace_id, agent_id, job_type, status, priority,
  payload_json, result_json, error, attempts, max_attempts, scheduled_at,
  claimed_by, claimed_at, started_at, completed_at, heartbeat_at,
  created_at, updated_at
`;

const TERMINAL_RUN_STATES = [
  "succeeded",
  "failed",
  "degraded",
  "cancelled",
  "waiting_for_review",
];

export class PgRunJobRepository {
  constructor(private readonly db: Queryable) {}

  static fromConfig(config: ControlPlaneConfig): PgRunJobRepository {
    if (!config.databaseUrl) {
      throw new Error("Run job repository requires CONTROL_PLANE_DATABASE_URL");
    }
    return new PgRunJobRepository(getDbPool(config.databaseUrl));
  }

  async getJob(jobId: string): Promise<RunJobRecord | null> {
    const result = await this.db.query<RunJobRecord>(
      `SELECT ${JOB_SELECT_COLUMNS}
         FROM jobs
        WHERE id = $1`,
      [jobId],
    );
    return result.rows[0] ?? null;
  }

  async claimNextAgentRun(
    workerId: string,
    now: Date = new Date(),
  ): Promise<RunJobRecord | null> {
    // The candidate CTE column is aliased so the RETURNING list (shared
    // JOB_SELECT_COLUMNS) stays unambiguous — with "FROM candidate" both
    // relations are in scope and a bare "id" makes PostgreSQL fail with
    // "column reference is ambiguous" (covered by runsIntegration.test.ts).
    const result = await this.db.query<RunJobRecord>(
      `WITH candidate AS (
         SELECT id AS candidate_job_id
           FROM jobs
          WHERE status = 'pending'
            AND scheduled_at <= $2
            AND attempts < max_attempts
            AND job_type = 'agent_run'
          ORDER BY priority DESC, scheduled_at ASC
          FOR UPDATE SKIP LOCKED
          LIMIT 1
       )
       UPDATE jobs
          SET status = 'claimed',
              claimed_by = $1,
              claimed_at = $2,
              heartbeat_at = NULL,
              attempts = attempts + 1,
              updated_at = $2
         FROM candidate
        WHERE jobs.id = candidate.candidate_job_id
        RETURNING ${JOB_SELECT_COLUMNS}`,
      [workerId, now.toISOString()],
    );
    return result.rows[0] ?? null;
  }

  async startJob(
    jobId: string,
    workerId: string | null,
    now: Date = new Date(),
  ): Promise<boolean> {
    const result = await this.db.query(
      `UPDATE jobs
          SET status = 'running',
              started_at = $2,
              heartbeat_at = $2,
              updated_at = $2
        WHERE id = $1
          AND status = 'claimed'
          AND (CAST($3 AS text) IS NULL OR claimed_by = $3)`,
      [jobId, now.toISOString(), workerId],
    );
    return (result.rowCount ?? 0) > 0;
  }

  async completeJob(
    jobId: string,
    resultJson: unknown,
    workerId: string | null,
    now: Date = new Date(),
  ): Promise<boolean> {
    const result = await this.db.query(
      `UPDATE jobs
          SET status = 'completed',
              result_json = $2::jsonb,
              completed_at = $3,
              heartbeat_at = NULL,
              updated_at = $3
        WHERE id = $1
          AND status IN ('claimed', 'running')
          AND (CAST($4 AS text) IS NULL OR claimed_by = $4)`,
      [
        jobId,
        JSON.stringify(sanitizeEvidenceJson(resultJson ?? {})),
        now.toISOString(),
        workerId,
      ],
    );
    return (result.rowCount ?? 0) > 0;
  }

  async failJob(
    jobId: string,
    error: string,
    workerId: string | null,
    now: Date = new Date(),
  ): Promise<RunJobStatus | null> {
    const result = await this.db.query<{ status: RunJobStatus }>(
      `UPDATE jobs
          SET status = CASE WHEN attempts < max_attempts THEN 'pending' ELSE 'failed' END,
              error = $2,
              heartbeat_at = NULL,
              claimed_by = CASE WHEN attempts < max_attempts THEN NULL ELSE claimed_by END,
              claimed_at = CASE WHEN attempts < max_attempts THEN NULL ELSE claimed_at END,
              started_at = CASE WHEN attempts < max_attempts THEN NULL ELSE started_at END,
              completed_at = CASE WHEN attempts < max_attempts THEN NULL ELSE $3 END,
              updated_at = $3
        WHERE id = $1
          AND status IN ('claimed', 'running')
          AND (CAST($4 AS text) IS NULL OR claimed_by = $4)
        RETURNING status`,
      [jobId, redactEvidenceText(error), now.toISOString(), workerId],
    );
    return result.rows[0]?.status ?? null;
  }

  async cancelJob(
    jobId: string,
    workerId: string | null,
    now: Date = new Date(),
  ): Promise<boolean> {
    const result = await this.db.query(
      `WITH cancelled_job AS (
         UPDATE jobs
            SET status = 'cancelled',
                heartbeat_at = NULL,
                completed_at = $2,
                updated_at = $2
          WHERE id = $1
            AND status IN ('pending', 'claimed', 'running')
            AND (CAST($3 AS text) IS NULL OR claimed_by = $3)
          RETURNING id, job_type, payload_json
       ),
       cancelled_run AS (
         UPDATE runs
            SET status = 'cancelled',
                ended_at = $2,
                updated_at = $2
           FROM cancelled_job
          WHERE cancelled_job.job_type = 'agent_run'
            AND runs.id::text = cancelled_job.payload_json->>'run_id'
            AND runs.status <> ALL($4::text[])
          RETURNING runs.id
       )
       SELECT id FROM cancelled_job`,
      [jobId, now.toISOString(), workerId, TERMINAL_RUN_STATES],
    );
    return (result.rowCount ?? 0) > 0;
  }

  async touchHeartbeat(
    jobId: string,
    workerId: string | null,
    now: Date = new Date(),
  ): Promise<boolean> {
    const result = await this.db.query(
      `UPDATE jobs
          SET heartbeat_at = $2,
              updated_at = $2
        WHERE id = $1
          AND status IN ('claimed', 'running')
          AND (CAST($3 AS text) IS NULL OR claimed_by = $3)`,
      [jobId, now.toISOString(), workerId],
    );
    return (result.rowCount ?? 0) > 0;
  }

  async appendJobEvent(input: {
    job_id: string;
    event_type: string;
    message: string;
    data?: unknown;
    created_at?: Date;
  }): Promise<RunJobEventRecord> {
    const result = await this.db.query<RunJobEventRecord>(
      `INSERT INTO job_events (id, job_id, event_type, message, data, created_at)
       VALUES ($1, $2, $3, $4, $5::jsonb, $6)
       RETURNING id, job_id, event_type, message`,
      [
        randomUUID(),
        input.job_id,
        input.event_type,
        redactEvidenceText(input.message) ?? "",
        JSON.stringify(sanitizeEvidenceJson(input.data ?? {})),
        (input.created_at ?? new Date()).toISOString(),
      ],
    );
    const row = result.rows[0];
    if (!row) throw new Error("JobEvent append returned no row");
    return row;
  }

  async reclaimStuckJobs(
    stuckAfterSeconds = 600,
    now: Date = new Date(),
  ): Promise<RunJobReclaimResult> {
    const cutoff = new Date(now.getTime() - stuckAfterSeconds * 1000).toISOString();
    await this.deleteOrphanRunExecutionLocks(cutoff);

    const result = await this.db.query<{ reclaimed_count: string | number }>(
      `WITH retryable AS (
         UPDATE jobs
            SET status = 'pending',
                claimed_by = NULL,
                claimed_at = NULL,
                started_at = NULL,
                heartbeat_at = NULL,
                updated_at = $1
          WHERE status IN ('claimed', 'running')
            AND COALESCE(heartbeat_at, updated_at) < $2
            AND attempts < max_attempts
          RETURNING id
       ),
       exhausted_agent_runs AS (
         SELECT payload_json->>'run_id' AS run_id
           FROM jobs
          WHERE status IN ('claimed', 'running')
            AND job_type = 'agent_run'
            AND COALESCE(heartbeat_at, updated_at) < $2
            AND attempts >= max_attempts
            AND payload_json ? 'run_id'
       ),
       failed AS (
         UPDATE jobs
            SET status = 'failed',
                claimed_by = NULL,
                claimed_at = NULL,
                heartbeat_at = NULL,
                completed_at = $1,
                error = 'job stuck and retry attempts exhausted',
                updated_at = $1
          WHERE status IN ('claimed', 'running')
            AND COALESCE(heartbeat_at, updated_at) < $2
            AND attempts >= max_attempts
          RETURNING id
       ),
       failed_runs AS (
         UPDATE runs
            SET status = 'failed',
                ended_at = $1,
                updated_at = $1,
                error_message = 'run abandoned: backing job stuck and retry attempts exhausted'
           FROM exhausted_agent_runs
          WHERE runs.id::text = exhausted_agent_runs.run_id
            AND runs.status <> ALL($3::text[])
          RETURNING runs.id
       )
       SELECT
         (SELECT COUNT(*) FROM retryable) + (SELECT COUNT(*) FROM failed)
           AS reclaimed_count`,
      [now.toISOString(), cutoff, TERMINAL_RUN_STATES],
    );
    const count = result.rows[0]?.reclaimed_count ?? 0;
    return { reclaimed_count: Number(count) };
  }

  private async deleteOrphanRunExecutionLocks(cutoff: string): Promise<void> {
    await this.db.query(
      `WITH stuck_agent_runs AS (
         SELECT id, payload_json->>'run_id' AS run_id
           FROM jobs
          WHERE status IN ('claimed', 'running')
            AND job_type = 'agent_run'
            AND COALESCE(heartbeat_at, updated_at) < $1
            AND payload_json ? 'run_id'
       )
       DELETE FROM run_execution_locks locks
        USING stuck_agent_runs
        WHERE locks.run_id::text = stuck_agent_runs.run_id
          AND (locks.job_id = stuck_agent_runs.id OR locks.job_id IS NULL)`,
      [cutoff],
    );
  }
}
