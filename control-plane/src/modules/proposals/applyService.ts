...
export class PgProposalApplyService {
  ...
  static fromConfig(config: ControlPlaneConfig): PgProposalApplyService {
  ...
  private async connect(): Promise<PoolClient> {
    if (!this.config.databaseUrl) {
      throw new Error("Proposal apply requires CONTROL_PLANE_DATABASE_URL");
    }
  ...
  private async enforceApplyPolicy(
    ...
    const result = await enforceProposalApply(
      this.config,
      {
        ...
        metadata_json: { control_plane_apply: true },
      },
      role,
