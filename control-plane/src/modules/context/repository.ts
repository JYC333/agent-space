...
/**
 * Repository for the full-run TS context.prepare path.
 *
...
export class PgRunContextRepository {
  ...
  static fromConfig(config: ControlPlaneConfig): PgRunContextRepository {
    if (!config.databaseUrl) {
      throw new Error("Run context repository requires CONTROL_PLANE_DATABASE_URL");
    }
    return new PgRunContextRepository(getDbPool(config.databaseUrl));
  }

  static poolFromConfig(config: ControlPlaneConfig): Pool {
    if (!config.databaseUrl) {
      throw new Error("Run context repository requires CONTROL_PLANE_DATABASE_URL");
    }
  ...
  async retrieve(input: {
    ...
    // Embedding fallback is currently a keyword delegation in Python; after the
    // keyword stage there are no new rows left to add from the same backend.
    stageTraces.push({
...
function resolveReadableScopes(
...
function assignSection(row: ContextMemoryRow): "stable_prefix" | "dynamic_tail" {
...
function policySourceRef(row: PolicyRow): Record<string, unknown> {
  return {
    source_type: "policy",
    source_id: row.id,
...
function evidenceSelectionFromRow(row: EvidenceContextRow): ContextEvidenceSelection {
...
function generatePersonalSummary(memories: readonly ContextMemoryRow[]): string {
