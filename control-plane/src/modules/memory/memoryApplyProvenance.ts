/**
 * Provenance + relation write helpers for the memory apply path (Stage 6 slice
 * 7b.2). Faithful port of `app.memory.provenance_apply` (`write_provenance_links`,
 * `copy_provenance_to_memory`, `record_memory_supersedes_relation`) and the
 * provenance helpers in `app.memory.proposal_payload` (`dominant_source_trust`,
 * `first_activity_id`, `merge_distinct_provenance_entries`,
 * `proposal_provenance_entry`, `user_confirmation_entry`).
 *
