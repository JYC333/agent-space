...
export class PgContextSnapshotRepository {
  ...
  static fromConfig(config: ControlPlaneConfig): PgContextSnapshotRepository {
    if (!config.databaseUrl) {
      throw new Error(
        "Context snapshot repository requires CONTROL_PLANE_DATABASE_URL",
      );
    }
  ...
  async persistChatSnapshot(input: ChatSnapshotPersistInput): Promise<void> {
    // Update the run's existing snapshot row (space-scoped). token_estimate and
    // request_json mirror the Python update branch; other audit fields stay as
    // create-run left them.
    await this.db.query(
