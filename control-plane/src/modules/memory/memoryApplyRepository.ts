/**
 * Memory proposal appliers (Stage 6 slice 7b.3 + 7b.4).
 *
 * Faithful port of `app.proposals.apply_service.MemoryProposalApplier`
 * (apply_create / apply_update / apply_archive) + the `MemoryInternalWriter` /
 * `MemoryStore.create` INSERT it drives. These run the durable active-memory
 ...
 *
 * SCOPE (7b.3/7b.4): the per-type write business logic only. The cross-cutting
 * accept orchestration — source-monitoring enforcement (7b.1 evaluator), the
 * personal-memory egress guard, digest invalidation, the proposal accept state
 * machine, and the apply-authority switch — is wired in 7b.5. Nothing here is
 * routed yet; the appliers are dormant.
 *
 * GAP carried to 7b.5: `check_private_memory_placement` is ported only as its
 * hard invariant (visibility=private requires a personal space). The
 * active-policy overlay (custom `memory.private_placement` deny / allow-with-log
 * + policy traces) lives in the Python policy subsystem and is not ported; the
 * flip gate must account for it.
 */
...
/** Raised when a memory proposal needs an apply capability the TS authority
...
// Payload markers that make a proposal grant-derived (mirror of
// `is_grant_derived_proposal`); plus any run context, which routes egress
// evaluation to the still-Python path.
...
export class PgMemoryApplyRepository {
   ...
   * validated pending/not-preview/space — this is the equivalent of
   * `ProposalApplyService.apply` + the accept state tail in `ProposalService
   * .accept`). accept_context is fixed `explicit_user_accept`.
   *
   * Fails closed (`MemoryApplyUnsupportedError`) for proposals needing apply
   * capabilities not yet ported: run/grant egress context (the personal-memory
   * egress guard) and workspace/agent-scope memory (digest invalidation). Those
   * stay Python-owned until 7b extends the TS path.
   */
  async acceptAndApply(proposal: ApplyProposal, userId: string): Promise<MemoryAcceptResult> {
    ...
    if (result.memory.scope_type === "workspace" || result.memory.scope_type === "agent") {
      throw new MemoryApplyUnsupportedError(
        `${result.memory.scope_type}-scope memory apply is not served by the TS authority yet (digest invalidation pending)`,
      );
    }
  ...
  private assertNoEgressContext(proposal: ApplyProposal): void {
    ...
    if (proposal.proposal_type === "egress_review") {
      throw new MemoryApplyUnsupportedError("egress_review apply is Python-owned");
    }
    if (proposal.created_by_run_id || strOr(payload.source_run_id)) {
      throw new MemoryApplyUnsupportedError(
        "memory proposals with run egress context are not served by the TS authority yet",
      );
    }
    for (const marker of GRANT_DERIVED_MARKERS) {
      if (payload[marker]) {
        throw new MemoryApplyUnsupportedError(
          "grant-derived memory proposals are not served by the TS authority yet",
        );
      }
    }
  }
