...
/**
 * Stage 6 slice 6: TS-owned public memory proposal creation. This repository
 * writes pending `proposals` rows only. It reads target memories for
 * update/archive authorization, but never mutates `memory_entries`; active
 * memory writes remain proposal-apply authority (slice 7).
 */
export class PgMemoryProposalRepository {
  ...
  static fromConfig(config: ControlPlaneConfig): PgMemoryProposalRepository {
    if (!config.databaseUrl) {
      throw new Error("Memory proposal repository requires CONTROL_PLANE_DATABASE_URL");
    }
