...
export function authRepositoryFromConfig(config: ControlPlaneConfig): AuthRepository | null {
...
export async function introspectIdentity(
  ...
  if (!repository) {
    request.log.warn("native identity requires CONTROL_PLANE_DATABASE_URL");
