/**
 * Policy decision core — TS port of the pure decision logic:
 *   - `hard_invariants.py` (HardInvariantGuard, 7 non-overridable invariants)
 *   - `roles.py` (canonical role ranks)
 *   - `rules.py` (8 built-in rules)
 *   - `engine.py` (rule evaluation + registry-default fallthrough)
 *
 * These are deterministic and DB-free. The orchestration (guard → engine,
 * audit-required, failure-mode) lives in `gateway.ts`. Keep this file in
 * lockstep with the Python sources.
 */
