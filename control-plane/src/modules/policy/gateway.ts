/**
 * Policy gateway compute logic — TS port of the orchestration in
 * `app/policy/gateway.py` and the pure parts of `proposal_apply.py`.
 *
 ...
 * (`service.ts`), preserving the §8 boundary that TS runs/policy never read
 ...
 * (registered applier ∩ risk table) — both resolved by the service from Python
 * ports / DB. Mirrors `check_proposal_apply_policy` exactly.
 */
