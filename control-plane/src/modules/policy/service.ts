/**
 * Policy enforcement service — TS port of `PolicyGateway.enforce` and
 * `enforce_proposal_apply` (the orchestration; the decision logic lives in
 * `gateway.ts` / `decisionCore.ts`).
 *
 * Unlike Python — where a blocking decision raises `PolicyGateBlocked` and a
 * global exception handler writes the durable record — the internal port writes
 * the durable audit itself and returns a structured `PolicyEnforceResult`
 ...
 * process, so this stays a plain TS interface (the wire type is a strict
...
async function persistAudit(
  ...
  if (!config.databaseUrl) {
    throw new Error("policy enforcement requires CONTROL_PLANE_DATABASE_URL");
  }
 ...
 * both resolved by the caller from Python-owned ports (the proposals phase wires
 * those ports; this function stays DB-free and faithful to
 * `enforce_proposal_apply`). proposal.apply is FAIL_CLOSED + audit_required, so
 * an ALLOW whose audit write fails returns `error`.
 */
