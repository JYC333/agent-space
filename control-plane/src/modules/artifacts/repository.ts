...
export class PgArtifactRepository {
  constructor(
    ...
    private readonly config: Pick<ControlPlaneConfig, "artifactStorageRoot" | "sandboxRoot">,
  ) {}

  static fromConfig(config: ControlPlaneConfig): PgArtifactRepository {
    if (!config.databaseUrl) {
      throw new Error("Artifact repository requires CONTROL_PLANE_DATABASE_URL");
    }
