...
const DEFAULT_EXECUTION_PLANES: readonly ExecutionPlaneSeed[] = [
  {
...
export async function seedSpaceDefaults(client: PoolClient, spaceId: string): Promise<void> {
  await seedExecutionPlanes(client, spaceId);
  await seedSystemMemories(client, spaceId);
  await seedNoteCollections(client, spaceId);
}

async function seedExecutionPlanes(client: PoolClient, spaceId: string): Promise<void> {
  for (const seed of DEFAULT_EXECUTION_PLANES) {
    await client.query(
      `INSERT INTO execution_planes
       ...
       SELECT $1::varchar(36), $2::varchar(36), $3::varchar(256),
              $4::varchar(32), $5::varchar(64), $6::varchar(32),
              $7::varchar(64), $8::varchar(32), $9::varchar(64),
              $10::varchar(64), $11::varchar(32), '{}'::jsonb,
              $12::boolean, now(), now()
       WHERE NOT EXISTS (
         SELECT 1 FROM execution_planes
          WHERE space_id = $2::varchar(36) AND name = $3::varchar(256)
       )`,
      [
        randomUUID(),
        spaceId,
        ...
        seed.type,
        seed.provider,
        seed.execution_location,
        seed.runtime_origin,
        seed.trust_level,
        seed.observability_level,
        seed.data_exposure_level,
