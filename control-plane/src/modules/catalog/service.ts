 ...
 * on disk and their declared summary fields. It is NOT the product Template
 * Library or the capability registry — Python's `/api/v1/agent-templates` and
 * `/api/v1/capabilities` remain the business authority (DB-backed versions,
 * visibility rules, enablement). Responses include declared fields like
 * `visibility` verbatim so consumers can filter; this module applies no policy.
 *
