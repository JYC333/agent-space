 ...
 * TS migration runner.
 *
 * Re-platform foundation: the control plane can replay and eventually evolve
 * the PostgreSQL schema. Until an explicit schema-ownership cutover,
 * Python/Alembic remains the runtime schema authority and still runs in prod;
 * this runner is proven in parity first.
 * `control-plane/migrations/0001_baseline.sql` is a frozen, faithful dump of the
 * Alembic head; future schema changes are added here as new `NNNN_*.sql` files.
 *
 * Design:
 * - Migrations are ordered `.sql` files named `NNNN_name.sql` under a directory;
 *   lexicographic filename order is apply order.
 * - Applied migrations are tracked in `public.control_plane_schema_migrations`
 ...
 * - The whole run holds one session-level **advisory lock**, so two control-plane
...
const MIGRATIONS_TABLE = "public.control_plane_schema_migrations";
