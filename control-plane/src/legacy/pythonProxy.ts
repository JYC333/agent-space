/**
 * **Temporary** transparent fallback proxy to the legacy Python authority.
 *
 * Any `/api/v1/*` request not owned by a TS control-plane route is forwarded
 * verbatim to the Python backend. Python remains the sole authority — the control
 * plane makes no decision about the request beyond routing it. This module is
 ...
 * dedicated SSE keep-alive handling are intentionally out of scope (see
 * TS_CONTROL_PLANE_FOUNDATION.md "known limitations").
 */
...
/** Returned when the legacy proxy is disabled but a fallback route is hit. */
export const LEGACY_PROXY_DISABLED: ErrorEnvelope = {
  error: "legacy_proxy_disabled",
  message: "Legacy Python proxy is disabled on this control plane",
};
...
/**
 * Register the catch-all legacy Python fallback for `/api/v1/*`. Must be
 * registered AFTER all TS-owned control-plane routes so that specific routes win.
 */
export function registerLegacyPythonProxy(
  app: FastifyInstance,
  config: ControlPlaneConfig,
): void {
  app.all("/api/v1/*", async (request, reply) => {
    if (!config.enableLegacyProxy) {
      return reply.code(503).send(LEGACY_PROXY_DISABLED);
    }

    const requestId = resolveRequestId(request);

    // request.url is the raw path + query string, forwarded unchanged.
    const upstreamUrl = `${config.legacyPythonApiBaseUrl}${request.url}`;
