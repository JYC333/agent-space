 ...
 * plane behind native TS identity. Uses fake DB/auth ports; PostgreSQL access is
...
describe("providers TS read authority", () => {
  it("serves list and detail from the DB port scoped by native TS identity", async () => {
    __setAuthIdentityForTests({ spaceId: "space-1", userId: "user-1" });
    __setProvidersDbPortForTests(
      fakeDb({ "space-1": [provider("mp-2"), provider("mp-1")] }),
    );
    app = buildServer(tsAuthorityConfig(), { logger: false });
  ...
  it("serves the catalog constant and litellm-providers from TS static catalogs", async () => {
    __setAuthIdentityForTests({ spaceId: "space-1", userId: "user-1" });
  ...
  it("advertises the fixed TS authority feature", async () => {
    __setAuthIdentityForTests({ spaceId: "space-1", userId: "user-1" });
    __setProvidersDbPortForTests(fakeDb({}));
    app = buildServer(tsAuthorityConfig(), { logger: false });
