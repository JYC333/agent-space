...
// Real-PostgreSQL integration tests for the memory appliers (Stage 6 slice
// 7b.3/7b.4). These run the actual INSERT/UPDATE memory_entries + provenance /
// relation writes against a throwaway Postgres loaded with memoryApplySchema.sql,
// so the column set, versioning, supersede, and placement invariant are exercised
// on the real stack. Skips gracefully when Docker is unavailable.
