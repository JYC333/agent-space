...
// Real-Postgres tests for the TS migration runner. Skips gracefully when Docker
...
let dir: string;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "cp-migrations-"));
});
afterEach(async () => {
...
describe("TS migration runner", () => {
  it("applies pending migrations, records versions, and is idempotent", async () => {
    ...
    const versions = await pool.query<{ version: string }>(
      "SELECT version FROM public.control_plane_schema_migrations ORDER BY version",
    );
  ...
  it("serializes concurrent runs via the advisory lock (no double-apply)", async () => {
    ...
    const versions = await pool.query<{ version: string }>(
      "SELECT version FROM public.control_plane_schema_migrations",
    );
