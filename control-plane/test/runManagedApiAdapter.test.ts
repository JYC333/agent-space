...
describe("executeManagedApiNoToolAdapter", () => {
  ...
  // in RunOrchestrationService.enforceRuntimePolicy via the Python policy.enforce
  // port — see runOrchestrationService.test.ts "fails closed before adapter
  // invocation when policy denies" and the backend contract tests. The adapter
  // no longer carries its own (previously unwired) policy seam.
