...
// Real-PostgreSQL integration tests for the TS context snapshot repository
// (Stage 6 slice 4). The route/parity unit suites use fakes that never run SQL,
// so they cannot catch the defects that only surface on the real stack: the
// Python-only default columns (id/metadata_json/created_at) the item INSERT must
// supply, the ck_context_snapshot_items_item_type CHECK, jsonb param binding,
// the space-scoped snapshot UPDATE, and the multi-row VALUES insert. These run
// the actual SQL against a throwaway Postgres (testcontainers) loaded with
// test/fixtures/contextSnapshotSchema.sql.
//
...
beforeEach(async () => {
  ...
  // Seed the empty snapshot the Python create-run port would have created.
  await pool.query(
