...
#
# Usage:
#   scripts/system/backup.sh [--mode dev|test|prod] [--output DIR] [--include-logs] [--force-running]
...
MANIFEST_APP_VERSION="$(grep -oE 'app_version: str = "[^"]+"' "$REPO_ROOT/core/backend/app/config.py" 2>/dev/null | grep -oE '"[^"]+"' | tr -d '"' || true)"
...
echo "[backup] restore with: scripts/system/restore.sh $ARCHIVE_PATH --mode $MODE"
