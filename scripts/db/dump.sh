...
# scripts/system/backup.sh or the BackupService API.
#
# Dumps are written to $ASPACE_ROOT/<mode>/db/dumps/ and restore with
# scripts/db/restore.sh or pg_restore.
#
# Usage:
#   scripts/db/dump.sh [--mode dev|test|prod] [output.dump]
...
echo "Restore with: scripts/db/restore.sh $OUTPUT_PATH --mode $MODE"
